...
Section BLS24_Pairing.
  ...
  (** Hard part using the addition chain decomposition:
      ...
      This requires:
        - 8 exponentiations by |u| (52-bit, via fp24_pow_z)
        - Frobenius maps (p, p^2, p^4)
        - Several Fp24 multiplications
      ...
      We compute step by step:
        ...
        b = a * f^{-1}         = f^{u-1}
        c = b^u
        d = c * b^{-1}         = f^{u(u-1) - (u-1)} = f^{(u-1)^2}
            (actually c * conj(b) since b is in cyclotomic subgroup)
        e = d^u                = f^{u*(u-1)^2}
        g = frob(d)            = f^{p*(u-1)^2}
        h = e * g              = f^{(u+p)*(u-1)^2}
        i = h^u                = f^{u*(u+p)*(u-1)^2}
        j = frob_p2(d)         = f^{p^2*(u-1)^2}
        k = i * j              = f^{(u^2+p^2)*(u-1)^2}
            wait... this isn't right, we need (u+p)*(u^2+p^2).

      Let us follow the standard approach more carefully:

        Step 1: t0 = f^u                 (exp by z, note z < 0)
        Step 2: t1 = t0 * conj(f)        = f^{u-1}   (since conj = f^{-1} cyclotomic)
        Step 3: t2 = t1^u                = f^{u(u-1)}
        Step 4: t3 = t2 * conj(t1)       = f^{u(u-1) - (u-1)} = f^{(u-1)^2}
        Step 5: t4 = t3^u                = f^{u(u-1)^2}
        Step 6: t5 = frob(t3)            = f^{p(u-1)^2}
        Step 7: t6 = t4 * t5             = f^{(u+p)(u-1)^2}
        Step 8: t7 = t6^u                = f^{u(u+p)(u-1)^2}
        Step 9: t8 = t7^u                = f^{u^2(u+p)(u-1)^2}
        Step 10: t9 = frob_p2(t6)        = f^{p^2(u+p)(u-1)^2}
        Step 11: t10 = t8 * t9           = f^{(u^2+p^2)(u+p)(u-1)^2}
        Step 12: t11 = t10^u             = f^{u(u^2+p^2)(u+p)(u-1)^2}
        Step 13: t12 = t11^u             = f^{u^2(u^2+p^2)(u+p)(u-1)^2}
        Step 14: t13 = t12^u             = f^{u^3(u^2+p^2)(u+p)(u-1)^2}
        Step 15: t14 = t13^u             = f^{u^4(u^2+p^2)(u+p)(u-1)^2}
        Step 16: t15 = frob_p4(t10)      = f^{p^4(u^2+p^2)(u+p)(u-1)^2}
        Step 17: t16 = t14 * t15         = f^{(u^4+p^4)(u^2+p^2)(u+p)(u-1)^2}
        Step 18: t17 = conj(t10)         = f^{-(u^2+p^2)(u+p)(u-1)^2}
        Step 19: t18 = t16 * t17         = f^{(u^4+p^4-1)(u^2+p^2)(u+p)(u-1)^2}
        Step 20: result = t18 * f^2 * f  = f^{(u^4+p^4-1)(u^2+p^2)(u+p)(u-1)^2 + 3}
  *)
  Definition final_exp_hard (f : Fp24) : Fp24 :=
    (* f^u (note: z < 0, so this uses conjugation internally) *)
    let t0 := fp24_pow_z f in
    (* f^{u-1} = f^u * f^{-1}; in cyclotomic subgroup, f^{-1} = conj(f) *)
    let t1 := fp24_mul p t0 (fp24_conjugate p f) in
    (* f^{u(u-1)} *)
    let t2 := fp24_pow_z t1 in
    (* f^{(u-1)^2} = f^{u(u-1)} * f^{-(u-1)} *)
