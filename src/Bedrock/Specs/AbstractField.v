...
Section FieldSpecs.
  ...
  Class FieldParameters_ok {F : Type} {field_parameters : FieldParameters F} := {
  ...
  Section FunctionSpecs.
    ...
    (* Nullary operations (zero, one) *)
    Class NullOp (name: string) :=
      { null_model : F;
        null_outbounds : bounds }.

    Definition nullop_spec {name} (op: NullOp name) :=
      fnspec! name (pout : word) / (out : felem) Rr,
      { requires tr mem :=
          (FElem pout out * Rr)%sep mem;
        ensures tr' mem' :=
          tr = tr' /\
          exists out',
            feval out' = null_model
            /\ bounded_by null_outbounds out'
            /\ (FElem pout out' * Rr)%sep mem' }.
  ...
  (* FieldNames: additional function name strings for zero/one *)
  Class FieldNames {F : Type} {field_parameters : FieldParameters F} :=
    { zero : string;
      one : string }.

  Definition field_names_prefixed
    {F : Type} {field_parameters : FieldParameters F}
    (prefix : string) : FieldNames :=
    {| zero := (prefix ++ "zero")%string;
       one := (prefix ++ "one")%string |}.

  Section NullOpInstances.
    Context {width: Z} {BW: Bitwidth width} {word: word.word width} {mem: map.map word Byte.byte}.
    Context {locals: map.map String.string word}.
    Context {env: map.map String.string (list String.string * list String.string * Syntax.cmd)}.
    Context {ext_spec: bedrock2.Semantics.ExtSpec}.
    ...
    Instance null_zero : NullOp zero :=
      {| null_model := Fzero; null_outbounds := loose_bounds |}.
    Instance null_one : NullOp one :=
      {| null_model := Fone; null_outbounds := loose_bounds |}.
  End NullOpInstances.

  Existing Instances spec_of_UnOp spec_of_BinOp bin_mul un_square bin_add bin_sub
          un_scmula24 un_inv spec_of_felem_copy spec_of_NullOp null_zero null_one.
