(** * BN254 Top-Level Optimal-Ate Pairing WP Proof
    Standalone WP correctness proof for bn254_pairing_dsd_optimal from BN254_Pairing.v,
    plus the top-level bn254_pairing_dsd_optimal_correct_standalone theorem.
*)
...
Section BN254_PairingTopOptimal.
    ...
    Instance spec_of_bn254_pairing_dsd_optimal : spec_of "bn254_pairing_dsd_optimal" :=
      fnspec! "bn254_pairing_dsd_optimal" (pout p_px p_py p_qx p_qy : word)
    ...
    Lemma bn254_pairing_dsd_optimal_ok :
      forall functions
        ...
        (HLoadG2 : forall pout (old_out : Fp2_felem) R tr m,
          (FElem_Fp2 pout old_out ⋆ R) m ->
          Semantics.call functions "bn254_load_gamma2_p2" tr m [pout]
            (fun tr' m' rets => rets = [] /\ tr = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout out ⋆ R) m'))
        (HLoadW : forall pout (old_out : Fp2_felem) R tr m,
          (FElem_Fp2 pout old_out ⋆ R) m ->
          Semantics.call functions "bn254_load_w_frob_p2_c1" tr m [pout]
            (fun tr' m' rets => rets = [] /\ tr = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout out ⋆ R) m'))
                ...
                (FElem_Fp12 po fe_out ⋆
                 (FElem_Fp12 pf f ⋆
                  (FElem_Fp2 pg1 g1 ⋆
                   (FElem_Fp2 pg2 g2 ⋆
                    (FElem_Fp2 pw w ⋆ R))))) m')),
      spec_of_bn254_pairing_dsd_optimal functions.
    Proof.
      intros.
      unfold spec_of_bn254_pairing_dsd_optimal.
      ...
      (* === Call 1: bn254_load_gamma1_p2(gamma1_p2) === *)
      repeat straightline.
      ...
      (* === Call 2: bn254_load_gamma2_p2(gamma2_p2) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
        eapply HLoadG2.
      ...
      (* === Call 3: bn254_load_w_frob_p2_c1(w_frob_p2_c1) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
        eapply HLoadW.
      ...
      (* === Call 4: bn254_miller_loop(tmp, p_x, p_y, q_x, q_y) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
      ...
      (* === Call 5: bn254_final_exp_dsd(out, tmp, g1, g2, w) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
    ...
    Theorem bn254_pairing_dsd_optimal_correct_standalone :
      forall functions tr mem pout p_px p_py p_qx p_qy
        (old_out : Fp12_felem) (p_x p_y : Fp_felem)
        (q_x q_y : Fp2_felem) Rr,
        map.get functions "bn254_pairing_dsd_optimal" = Some (snd bn254_pairing_dsd_optimal) ->
        (* Loader hypotheses *)
        (forall pout0 (old_out0 : Fp2_felem) R tr0 m0,
          (FElem_Fp2 pout0 old_out0 ⋆ R) m0 ->
          Semantics.call functions "bn254_load_gamma1_p2" tr0 m0 [pout0]
        ...
        (forall pout0 (old_out0 : Fp2_felem) R tr0 m0,
          (FElem_Fp2 pout0 old_out0 ⋆ R) m0 ->
          Semantics.call functions "bn254_load_gamma2_p2" tr0 m0 [pout0]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout0 out ⋆ R) m')) ->
        (forall pout0 (old_out0 : Fp2_felem) R tr0 m0,
          (FElem_Fp2 pout0 old_out0 ⋆ R) m0 ->
          Semantics.call functions "bn254_load_w_frob_p2_c1" tr0 m0 [pout0]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout0 out ⋆ R) m')) ->
        ...
        (forall ptmp ppx ppy pqx pqy
          ...
          Semantics.call functions "bn254_miller_loop_optimal" tr0 m0
            [ptmp; ppx; ppy; pqx; pqy]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists ml_out,
                Fp12_bounded Fp12_tight ml_out /\
        ...
        (forall po pf pg1 pg2 pw
          ...
          Semantics.call functions "bn254_final_exp_dsd" tr0 m0
            [po; pf; pg1; pg2; pw]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists fe_out,
                Fp12_bounded Fp12_loose fe_out /\
        ...
        (FElem_Fp12 pout old_out ⋆
         (FElem_Fp p_px p_x ⋆
          (FElem_Fp p_py p_y ⋆
           (FElem_Fp2 p_qx q_x ⋆
            (FElem_Fp2 p_qy q_y ⋆ Rr))))) mem ->
        WeakestPrecondition.call functions "bn254_pairing_dsd_optimal" tr mem
          [pout; p_px; p_py; p_qx; p_qy]
          (fun tr' mem' rets =>
             rets = [] /\ tr = tr' /\
             (* Strengthened postcondition (was vacuous; see PLAN_PAIRING_SPECS.md):
                the call writes a bounded Fp12 element [out_val] at [pout] and
                preserves the input arguments. This is still NOT a mathematical
                spec — the next step is to add [feval out_val = optimal_ate ...]
                here, which requires the L1 model from PairingTheory/. *)
             exists out_val,
               Fp12_bounded Fp12_loose out_val /\
               (FElem_Fp12 pout out_val ⋆
                (FElem_Fp p_px p_x ⋆
                 (FElem_Fp p_py p_y ⋆
                  (FElem_Fp2 p_qx q_x ⋆
                   (FElem_Fp2 p_qy q_y ⋆ Rr))))) mem').
    Proof.
      intros functions tr mem pout p_px p_py p_qx p_qy
        ...
        Hbqx Hbqy Hbpx Hbpy Hsep.
      pose proof (bn254_pairing_dsd_optimal_ok functions HEnv HLG1 HLG2 HLW HML HFE
      ...
      intros tr' mem' rets [Hrets [Htr [out_val [Hbnd Hmem]]]].
      split; [exact Hrets | split; [exact Htr |]].
      exists out_val. split; [exact Hbnd | exact Hmem].
    Qed.

End BN254_PairingTopOptimal.
