(** * BN446 DSD Final Exponentiation Hard Part WP Proof
    Proves bn446_final_exp_hard_dsd (35 calls, 7 stackallocs) satisfies its spec.
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.BN446_PowU.
...
Section BN446_FinalExpHardDSD.
    ...
    Instance spec_of_Fp12_conjugate : spec_of fp12_conjugate_name :=
      AbstractField.unop_spec (F:=Fp12) (field_representation:=bn446_Fp12_rep')
        (@DodecicFieldExtensions.un_Fp12_conjugate _ _ _ _
          bn446_pf_params bn446_Fp_rep bn446_beta bn446_xi_re bn446_xi_im fp12_prefix fp6_prefix fp2_prefix).
    ...
    Definition spec_of_pow_u_inplace (functions : @map.rep String.string
      ...
      forall pbase (base_val : Fp12_felem) Rr tr mem,
        Fp12_bounded Fp12_tight base_val ->
        (FElem_Fp12 pbase base_val ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn446_Fp12_pow_u"
          tr mem [pbase; pbase]
          (fun tr' mem' rets =>
    ...
    Instance spec_of_bn446_load_gamma1 : spec_of "bn446_load_gamma1" :=
      fnspec! "bn446_load_gamma1" (pout : word)
    ...
    Instance spec_of_bn446_load_gamma2 : spec_of "bn446_load_gamma2" :=
      fnspec! "bn446_load_gamma2" (pout : word)
        / (old_out : Fp2_felem) Rr,
      { requires tr mem := (FElem_Fp2 pout old_out ⋆ Rr) mem;
    ...
    Instance spec_of_bn446_load_w_frob_c1 : spec_of "bn446_load_w_frob_c1" :=
      fnspec! "bn446_load_w_frob_c1" (pout : word)
        / (old_out : Fp2_felem) Rr,
      { requires tr mem := (FElem_Fp2 pout old_out ⋆ Rr) mem;
    ...
    Instance spec_of_bn446_final_exp_hard_dsd : spec_of "bn446_final_exp_hard_dsd" :=
      fnspec! "bn446_final_exp_hard_dsd" (pout pf : word)
    ...
    Local Instance bn446_Fp12_rep_ok' :
      @AbstractField.FieldRepresentation_ok _ bn446_Fp12_params' _ _ _ _ bn446_Fp12_rep' :=
      DodecicFieldExtensionsSpecs.Fp12_field_representation_ok bn446_beta bn446_xi_re bn446_xi_im
        (fp12_prefix:=fp12_prefix) (fp6_prefix:=fp6_prefix) (fp2_prefix:=fp2_prefix).

    Local Instance bn446_Fp2_rep_ok' :
      @AbstractField.FieldRepresentation_ok _ bn446_Fp2_params' _ _ _ _ bn446_Fp2_rep' :=
      QuadraticFieldExtensionsSpecs.Fp2_field_representation_ok bn446_beta fp2_prefix.

    Lemma bn446_final_exp_hard_dsd_ok :
      forall functions
        (EnvContains : map.get functions "bn446_final_exp_hard_dsd" =
          Some (snd BN446_Pairing.bn446_final_exp_hard_dsd))
        ...
        (HFloadg1 : spec_of_bn446_load_gamma1 functions)
        (HFloadg2 : spec_of_bn446_load_gamma2 functions)
        (HFloadw : spec_of_bn446_load_w_frob_c1 functions),
      spec_of_bn446_final_exp_hard_dsd functions.
    Proof.
      intros. unfold spec_of_bn446_final_exp_hard_dsd.
      ...
      unfold BN446_Pairing.bn446_final_exp_hard_dsd. simpl snd. simpl fst. cbv match beta.
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp12_params' _ _ _ _
        bn446_Fp12_rep' wordok mapok a_t0 mS_t0) HaS_t0) as [t0i Ht0i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp12_params' _ _ _ _
        bn446_Fp12_rep' wordok mapok a_t1 mS_t1) HaS_t1) as [t1i Ht1i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp12_params' _ _ _ _
        bn446_Fp12_rep' wordok mapok a_t2 mS_t2) HaS_t2) as [t2i Ht2i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp12_params' _ _ _ _
        bn446_Fp12_rep' wordok mapok a_t3 mS_t3) HaS_t3) as [t3i Ht3i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp2_params' _ _ _ _
        bn446_Fp2_rep' wordok mapok a_gamma1 mS_g1) HaS_g1) as [g1i Hg1i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp2_params' _ _ _ _
        bn446_Fp2_rep' wordok mapok a_gamma2 mS_g2) HaS_g2) as [g2i Hg2i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp2_params' _ _ _ _
        bn446_Fp2_rep' wordok mapok a_wfc1 mS_w) HaS_w) as [wi Hwi].

      unfold BN446_Pairing.final_exp_hard_dsd_body, BN446_Pairing.cmd_seq_list.
      ...
      (* ================================================================ *)
      (* Call 1: bn446_load_gamma1(gamma1)                                *)
      (* ================================================================ *)
      repeat straightline.
      ...
      (* ================================================================ *)
      (* Call 2: bn446_load_gamma2(gamma2)                                *)
      (* ================================================================ *)
      repeat straightline.
      eapply Semantics.weaken_call.
      ...
      (* ================================================================ *)
      (* Call 3: bn446_load_w_frob_c1(w_frob_c1)                         *)
      (* ================================================================ *)
      repeat straightline.
      eapply Semantics.weaken_call.
      ...
      { pose proof (AbstractField.FElem_to_bytes
                      (field_representation:=bn446_Fp2_rep')
      ...
      { pose proof (AbstractField.FElem_to_bytes
                      (field_representation:=bn446_Fp12_rep')
