(** * BN256 Pairing Helper WP Proofs
    Standalone WP correctness proofs for pairing helper functions
    defined in BN256_Pairing.v:
    - C1: bn256_Fp2_mul_fp (multiply Fp2 by Fp scalar)
    - C3: bn256_load_gamma1_p2
    - C4: bn256_load_gamma2_p2
    - C5: bn256_load_w_frob_p2_c1
    - C2: bn256_make_line
*)
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.bn256_prime.
Require Import Crypto.Bedrock.Field.Synthesis.Examples.bn256_prime_certif.
Require Import Crypto.Bedrock.Field.Synthesis.Examples.bn256_felem_copy.
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.BN256_Pairing.
...
(* ================================================================ *)
(* BN256 Section context — mirrors BN256_Pairing.v                  *)
(* ================================================================ *)

Section BN256_PairingHelpers.
    ...
    Local Notation Fp_feval := (@AbstractField.feval _ _ _ _ _ _ bn256_Fp_rep).
    Local Notation Fp2_feval := (@AbstractField.feval _ bn256_Fp2_params' _ _ _ _ bn256_Fp2_rep').
    ...
    Local Notation Fp_tight := (@AbstractField.tight_bounds _ _ _ _ _ _ bn256_Fp_rep).
    ...
    Local Notation fp_felem_offset :=
      (Memory.bytes_per_word 64 * Z.of_nat (@AbstractField.felem_size_in_words _ _ _ _ _ _ bn256_Fp_rep)).

    Local Notation fst_felem := (@QuadraticFieldExtensionsSpecs.fst_felem _ _ _ _ bn256_pf_params bn256_Fp_rep).
    Local Notation snd_felem := (@QuadraticFieldExtensionsSpecs.snd_felem _ _ _ _ bn256_pf_params bn256_Fp_rep).

    (* ============================================================ *)
    (* Compatibility: function body identity                         *)
    (* ============================================================ *)

    (* The function body defined here (via bn256_pf_params) must equal
       the imported one from BN256_Pairing (via bn256_prime_params).
       Both define M_pos = Z.to_pos bn256_prime.m and the same function names,
    ...
    Instance spec_of_Fp_mul : spec_of PrimeField.mul :=
      AbstractField.binop_spec (F:=Fp) (field_representation:=bn256_Fp_rep) AbstractField.bin_mul.

    Instance spec_of_Fp_felem_copy : spec_of (AbstractField.felem_copy (F:=Fp)) :=
      AbstractField.spec_of_felem_copy (F:=Fp) (field_representation:=bn256_Fp_rep).
    ...
    Instance spec_of_Fp_opp : spec_of (@AbstractField.opp _ prime_field_parameters) :=
      AbstractField.unop_spec (F:=Fp) (field_parameters:=prime_field_parameters)
        (field_representation:=bn256_Fp_rep) AbstractField.un_opp.

    Instance spec_of_Fp2_mul : spec_of (AbstractField.mul (F:=Fp2)) :=
      AbstractField.binop_spec (F:=Fp2) (field_representation:=bn256_Fp2_rep') AbstractField.bin_mul.

    Instance spec_of_Fp2_sub : spec_of (AbstractField.sub (F:=Fp2)) :=
      AbstractField.binop_spec (F:=Fp2) (field_representation:=bn256_Fp2_rep') AbstractField.bin_sub.

    Instance spec_of_Fp2_opp : spec_of (AbstractField.opp (F:=Fp2)) :=
      AbstractField.unop_spec (F:=Fp2) (field_representation:=bn256_Fp2_rep') AbstractField.un_opp.

    Instance spec_of_Fp2_felem_copy : spec_of (AbstractField.felem_copy (F:=Fp2)) :=
      AbstractField.spec_of_felem_copy (F:=Fp2) (field_representation:=bn256_Fp2_rep').

    Instance spec_of_Fp_from_word : spec_of PrimeField.from_word :=
      PrimeField.spec_of_from_word (field_representation:=bn256_Fp_rep).
    ...
    Proof.
      ...
      pose proof (QuadraticFieldExtensions.Fp2_raw_FElem_split bn256_beta
    ...
    Lemma FElem_Fp_join_in_sep p (a b : Fp_felem) R m :
      length a = @AbstractField.felem_size_in_words _ _ _ _ _ _ bn256_Fp_rep ->
      length b = @AbstractField.felem_size_in_words _ _ _ _ _ _ bn256_Fp_rep ->
      (FElem_Fp p a ⋆
    ...
    Proof.
      ...
      pose proof (QuadraticFieldExtensions.Fp2_raw_FElem_join bn256_beta
    ...
    Instance spec_of_bn256_Fp2_mul_fp : spec_of "bn256_Fp2_mul_fp" :=
      fnspec! "bn256_Fp2_mul_fp" (pout px ps : word)
    ...
    Lemma bn256_Fp2_mul_fp_ok :
      forall functions
        (EnvContains : map.get functions "bn256_Fp2_mul_fp" =
          Some (snd bn256_Fp2_mul_fp))
        (HFmul : spec_of_Fp_mul functions),
      spec_of_bn256_Fp2_mul_fp functions.
    Proof.
      intros.
      unfold spec_of_bn256_Fp2_mul_fp.
      ...
      unfold bn256_Fp2_mul_fp. simpl snd. simpl fst.
      cbv match beta.
      eexists. split. { exact eq_refl. }
      (* Step through: cmd.seq -> cmd.call arg evaluation *)
      straightline. straightline. straightline.
      (* Now at: call functions "bn256_mul" tr mem0 args postcondition *)
      ...
      pose proof (@QuadraticFieldExtensions.Fp2_raw_FElem_split _ _ _ _
        wordok mapok bn256_pf_params bn256_Fp_rep bn256_beta fp2_prefix
        px x m_x Hfx)
      ...
      change (@AbstractField.bounded_by _ bn256_Fp2_params' _ _ _ _ bn256_Fp2_rep')
        with (fun b ws => @AbstractField.bounded_by _ _ _ _ _ _ bn256_Fp_rep b
          (fst_felem ws)
          /\ @AbstractField.bounded_by _ _ _ _ _ _ bn256_Fp_rep b
          (snd_felem ws)) in Hbx.
      ...
      (* === Call 1: bn256_mul(out, x, s) — fst halves === *)
      ...
      1: { pose proof HFmul as HFmul1.
           ...
           split; [apply (@AbstractField.relax_bounds _ _ _ _ _ _
             bn256_Fp_rep bn256_Fp_rep_ok); exact Hbx1 |].
           split; [exact Hbs |].
      ...
      (* === Call 2: bn256_mul(out+off, x+off, s) — snd halves === *)
      eapply Semantics.weaken_call.
      1: { pose proof HFmul as HFmul2.
           ...
           split; [apply (@AbstractField.relax_bounds _ _ _ _ _ _
             bn256_Fp_rep bn256_Fp_rep_ok); exact Hbx2 |].
           split; [exact Hbs |].
      ...
      assert (Hlen_out1 : length out1 =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bn256_Fp_rep).
      { destruct Hsep1 as [mc [_ [_ [Hfc _]]]].
        exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bn256_pf_params bn256_Fp_rep _ _ _ Hfc). }
      ...
      assert (Hlen_x1 : length (fst_felem x) =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bn256_Fp_rep).
      { exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bn256_pf_params bn256_Fp_rep _ _ _ Hx1). }
      assert (Hlen_x2 : length (snd_felem x) =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bn256_Fp_rep).
      { exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bn256_pf_params bn256_Fp_rep _ _ _ Hx2). }
      ...
      { (* Decompose Hsep2 to get individual maps *)
        ...
        rewrite (@QuadraticFieldExtensions.Fp2_list_decomp _ _ _ _
          bn256_pf_params bn256_Fp_rep x) in Hfp2_x.
        (* Build final sep *)
    ...
    Local Ltac solve_store_fp2_constant :=
      ...
      [ (* Sep: prove via Bignum_of_array_sep to avoid emp issues *)
        ...
        change (@AbstractField.felem_size_in_words
          _ bn256_Fp2_params' _ _ _ _ bn256_Fp2_rep') with 8%nat;
    ...
    Lemma bn256_load_gamma1_p2_ok :
      forall functions
        (EnvContains : map.get functions "bn256_load_gamma1_p2" =
          Some (snd bn256_load_gamma1_p2)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn256_load_gamma1_p2" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn256_load_gamma1_p2. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
    ...
    Lemma bn256_load_gamma2_p2_ok :
      forall functions
        (EnvContains : map.get functions "bn256_load_gamma2_p2" =
          Some (snd bn256_load_gamma2_p2)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn256_load_gamma2_p2" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn256_load_gamma2_p2. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
      solve_store_fp2_constant.
    Qed.
    ...
    Lemma bn256_load_w_frob_p2_c1_ok :
      forall functions
        (EnvContains : map.get functions "bn256_load_w_frob_p2_c1" =
          Some (snd bn256_load_w_frob_p2_c1)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn256_load_w_frob_p2_c1" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn256_load_w_frob_p2_c1. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
      solve_store_fp2_constant.
    Qed.
    ...
    Local Notation fp2_felem_offset :=
      (Memory.bytes_per_word 64 * Z.of_nat (@AbstractField.felem_size_in_words _ bn256_Fp2_params' _ _ _ _ bn256_Fp2_rep')).
    Local Notation Fp6_felem_size := (@AbstractField.felem_size_in_words _ bn256_Fp6_params' _ _ _ _ bn256_Fp6_rep').
    Local Notation fp6_felem_offset :=
      (Memory.bytes_per_word 64 * Z.of_nat Fp6_felem_size).

    (* Fp6 sub-component access *)
    Local Notation c0_felem := (@CubicFieldExtensionsSpecs.c0_felem _ _ _ _ bn256_pf_params bn256_Fp_rep).
    Local Notation c1_felem := (@CubicFieldExtensionsSpecs.c1_felem _ _ _ _ bn256_pf_params bn256_Fp_rep).
    Local Notation c2_felem := (@CubicFieldExtensionsSpecs.c2_felem _ _ _ _ bn256_pf_params bn256_Fp_rep).
    (* Fp12 sub-component access *)
    Local Notation d0_felem := (@DodecicFieldExtensionsSpecs.d0_felem _ _ _ _ bn256_pf_params bn256_Fp_rep).
    Local Notation d1_felem := (@DodecicFieldExtensionsSpecs.d1_felem _ _ _ _ bn256_pf_params bn256_Fp_rep).
    (* Fp6_c1/c2 offsets computed with the correct (Fp-level) representation *)
    Local Notation fp6_c1_off :=
      (@CubicFieldExtensions.fp6_c1_offset _ _ _ _ bn256_pf_params bn256_beta bn256_Fp_rep fp2_prefix).
    Local Notation fp6_c2_off :=
      (@CubicFieldExtensions.fp6_c2_offset _ _ _ _ bn256_pf_params bn256_beta bn256_Fp_rep fp2_prefix).

    Lemma bn256_make_line_ok :
      forall functions
        (EnvContains : map.get functions "bn256_make_line" =
          Some (snd bn256_make_line))
        (HFp2mul : spec_of_Fp2_mul functions)
        (HFp2sub : spec_of_Fp2_sub functions)
        (HFp2opp : spec_of_Fp2_opp functions)
        (HFp2mulfp : spec_of_bn256_Fp2_mul_fp functions)
        (HFpcopy : spec_of_Fp_felem_copy functions)
        (HFfromword : spec_of_Fp_from_word functions),
      forall pout plam pxt pyt pxp pyp
        (old_out : @AbstractField.felem _ bn256_Fp12_params' _ _ _ _ bn256_Fp12_rep')
        ...
        WeakestPrecondition.call functions "bn256_make_line" tr mem
          [pout; plam; pxt; pyt; pxp; pyp]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              @AbstractField.bounded_by _ bn256_Fp12_params' _ _ _ _ bn256_Fp12_rep'
                (@AbstractField.loose_bounds _ bn256_Fp12_params' _ _ _ _ bn256_Fp12_rep') out /\
              (FElem_Fp12 pout out ⋆
               (FElem_Fp2 plam lam ⋆
                (FElem_Fp2 pxt xt ⋆
                 (FElem_Fp2 pyt yt ⋆
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn256_make_line. simpl snd. simpl fst.
      cbv match beta.
      eexists. split. { exact eq_refl. }
      repeat straightline.
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bn256_Fp2_params' _ _ _ _ bn256_Fp2_rep'
      ...
      pose proof (DodecicFieldExtensions.Fp12_raw_FElem_split bn256_beta bn256_xi_re bn256_xi_im
      ...
      pose proof (CubicFieldExtensions.Fp6_raw_FElem_split bn256_beta bn256_xi_re bn256_xi_im
      ...
      set (FE2 := @AbstractField.FElem _
        (Fp2_field_parameters bn256_beta fp2_prefix) _ _ _ _
        (Fp2_field_representation bn256_beta fp2_prefix)).
      ...
      change (Fp2_field_parameters bn256_beta fp2_prefix)
        with bn256_Fp2_params' in Hsep.
      change (Fp2_field_representation bn256_beta fp2_prefix)
        with bn256_Fp2_rep' in Hsep.

      (* Fp2-level relax_bounds helper *)
      pose proof (@Fp2_field_representation_ok _ _ _ _ bn256_pf_params
        bn256_Fp_rep bn256_Fp_rep_ok bn256_beta fp2_prefix) as Fp2_rep_ok.

      (* === 13 call steps: 4 Fp2-level + 9 Fp-level === *)
      (* Unfold cmd_seq_list so straightline can process cmd.seq *)
      unfold BN256_Pairing.cmd_seq_list.
      (* Unfold expression helpers so straightline can evaluate args *)
      unfold BN256_Pairing.expr_fp12_c0, BN256_Pairing.expr_fp12_c1,
             BN256_Pairing.expr_fp6_c0, BN256_Pairing.expr_fp6_c1,
             BN256_Pairing.expr_fp6_c2, BN256_Pairing.expr_fp_snd.
      ...
      1: { pose proof HFp2mul as HF1.
           ...
           split; [apply (@AbstractField.relax_bounds _ bn256_Fp2_params' _ _ _ _
             bn256_Fp2_rep' Fp2_rep_ok); exact Hblam |].
           split; [apply (@AbstractField.relax_bounds _ bn256_Fp2_params' _ _ _ _
             bn256_Fp2_rep' Fp2_rep_ok); exact Hbxt |].
      ...
      1: { pose proof HFp2mulfp as HF3.
           unfold spec_of_bn256_Fp2_mul_fp in HF3.
      ...
      *)
      ...
      Local Notation Fp_fsw := (@AbstractField.felem_size_in_words _ _ _ _ _ _ bn256_Fp_rep).
      pose proof fun p v m (H : FElem_Fp p v m) =>
        @QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bn256_pf_params bn256_Fp_rep p v m H
        as FpLen.
      ...
      Local Notation Fp2_felem_size := (@AbstractField.felem_size_in_words _ bn256_Fp2_params' _ _ _ _ bn256_Fp2_rep').
      ...
      pose proof (@CubicFieldExtensions.Fp6_raw_FElem_join _ _ _ _
        wordok mapok bn256_pf_params bn256_beta bn256_xi_re bn256_xi_im bn256_Fp_rep fp6_prefix fp2_prefix
      ...
      Local Notation Fp6_fsw := (@AbstractField.felem_size_in_words _ bn256_Fp6_params' _ _ _ _ bn256_Fp6_rep').
      ...
      pose proof (@DodecicFieldExtensions.Fp12_raw_FElem_join _ _ _ _
        wordok mapok bn256_pf_params bn256_Fp_rep bn256_beta bn256_xi_re bn256_xi_im fp12_prefix fp6_prefix fp2_prefix
      ...
      { (* bounded_by loose_bounds the_out *)
        ...
        unfold Fp2_bounded, AbstractField.bounded_by,
               bn256_Fp2_rep', bn256_Fp2_params',
        ...
        pose proof (@AbstractField.relax_bounds _ _ _ _ _ _
          bn256_Fp_rep bn256_Fp_rep_ok) as RB.
        ...
        Local Typeclasses Transparent bn256_Fp12_params'.
        Local Typeclasses Transparent bn256_Fp12_rep'.
        Local Typeclasses Transparent bn256_Fp6_params'.
        Local Typeclasses Transparent bn256_Fp6_rep'.
        Local Typeclasses Transparent bn256_Fp2_params'.
        Local Typeclasses Transparent bn256_Fp2_rep'.
        cbv [AbstractField.bounded_by AbstractField.loose_bounds
             bn256_Fp12_rep' bn256_Fp12_params'
             bn256_Fp6_rep' bn256_Fp6_params'
             bn256_Fp2_rep' bn256_Fp2_params'
             bn256_beta bn256_xi_re bn256_xi_im
        ...
        split.
        (* d0 bound *)
        { change (DodecicFieldExtensionsSpecs.Fp6_repr_inst bn256_beta bn256_xi_re bn256_xi_im) with bn256_Fp6_rep'.
          ...
          cbv [DodecicFieldExtensionsSpecs.Fp6_repr_inst bn256_beta bn256_xi_re bn256_xi_im
               CubicFieldExtensionsSpecs.Fp6_field_representation
               CubicFieldExtensionsSpecs.Fp6_field_parameters
               CubicFieldExtensionsSpecs.c0_felem
               CubicFieldExtensionsSpecs.c1_felem
