(** * BN446 Top-Level Pairing WP Proof
    Standalone WP correctness proof for bn446_pairing_dsd from BN446_Pairing.v,
    plus the top-level bn446_pairing_dsd_correct_standalone theorem.
*)
...
Section BN446_PairingTop.
    ...
    Instance spec_of_bn446_pairing_dsd : spec_of "bn446_pairing_dsd" :=
      fnspec! "bn446_pairing_dsd" (pout p_px p_py p_qx p_qy : word)
    ...
    Lemma bn446_pairing_dsd_ok :
      forall functions
        (EnvContains : map.get functions "bn446_pairing_dsd" =
          Some (snd bn446_pairing_dsd))
        (* Loader hypotheses: provide Semantics.call directly *)
        (HLoadG1 : forall pout (old_out : Fp2_felem) R tr m,
          (FElem_Fp2 pout old_out ⋆ R) m ->
          Semantics.call functions "bn446_load_gamma1_p2" tr m [pout]
        ...
        (HLoadG2 : forall pout (old_out : Fp2_felem) R tr m,
          (FElem_Fp2 pout old_out ⋆ R) m ->
          Semantics.call functions "bn446_load_gamma2_p2" tr m [pout]
            (fun tr' m' rets => rets = [] /\ tr = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout out ⋆ R) m'))
        (HLoadW : forall pout (old_out : Fp2_felem) R tr m,
          (FElem_Fp2 pout old_out ⋆ R) m ->
          Semantics.call functions "bn446_load_w_frob_p2_c1" tr m [pout]
            (fun tr' m' rets => rets = [] /\ tr = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout out ⋆ R) m'))
        ...
        (HMillerLoop : forall ptmp ppx ppy pqx pqy
          ...
          Semantics.call functions "bn446_miller_loop" tr m
        ...
        (HFinalExp : forall po pf pg1 pg2 pw
          ...
          Semantics.call functions "bn446_final_exp_dsd" tr m
            [po; pf; pg1; pg2; pw]
            (fun tr' m' rets => rets = [] /\ tr = tr' /\
              exists fe_out,
                ...
                (FElem_Fp12 po fe_out ⋆
                 (FElem_Fp12 pf f ⋆
                  (FElem_Fp2 pg1 g1 ⋆
                   (FElem_Fp2 pg2 g2 ⋆
                    (FElem_Fp2 pw w ⋆ R))))) m')),
      spec_of_bn446_pairing_dsd functions.
    Proof.
      intros.
      unfold spec_of_bn446_pairing_dsd.
      ...
      unfold bn446_pairing_dsd. simpl snd. simpl fst.
      cbv match beta.
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bn446_Fp12_params' _ _ _ _
        bn446_Fp12_rep' wordok mapok a_tmp) as Hfb_tmp.
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bn446_Fp2_params' _ _ _ _
        bn446_Fp2_rep' wordok mapok a_g1) as Hfb_g1.
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bn446_Fp2_params' _ _ _ _
        bn446_Fp2_rep' wordok mapok a_g2) as Hfb_g2.
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bn446_Fp2_params' _ _ _ _
        bn446_Fp2_rep' wordok mapok a_w) as Hfb_w.
      ...
      unfold BN446_Pairing.pairing_dsd_body, BN446_Pairing.cmd_seq_list.

      (* === Call 1: bn446_load_gamma1_p2(gamma1_p2) === *)
      repeat straightline.
      ...
      (* === Call 2: bn446_load_gamma2_p2(gamma2_p2) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
        eapply HLoadG2.
      ...
      (* === Call 3: bn446_load_w_frob_p2_c1(w_frob_p2_c1) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
        eapply HLoadW.
      ...
      (* === Call 4: bn446_miller_loop(tmp, p_x, p_y, q_x, q_y) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
      ...
      (* === Call 5: bn446_final_exp_dsd(out, tmp, g1, g2, w) === *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: {
    ...
    Theorem bn446_pairing_dsd_correct_standalone :
      forall functions tr mem pout p_px p_py p_qx p_qy
        ...
        (forall pout0 (old_out0 : Fp2_felem) R tr0 m0,
          (FElem_Fp2 pout0 old_out0 ⋆ R) m0 ->
          Semantics.call functions "bn446_load_gamma2_p2" tr0 m0 [pout0]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout0 out ⋆ R) m')) ->
        (forall pout0 (old_out0 : Fp2_felem) R tr0 m0,
          (FElem_Fp2 pout0 old_out0 ⋆ R) m0 ->
          Semantics.call functions "bn446_load_w_frob_p2_c1" tr0 m0 [pout0]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists out,
                Fp2_bounded Fp2_tight out /\
                (FElem_Fp2 pout0 out ⋆ R) m')) ->
        ...
        (forall ptmp ppx ppy pqx pqy
          ...
          Semantics.call functions "bn446_miller_loop" tr0 m0
            [ptmp; ppx; ppy; pqx; pqy]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists ml_out,
                Fp12_bounded Fp12_tight ml_out /\
        ...
        (forall po pf pg1 pg2 pw
          ...
          Semantics.call functions "bn446_final_exp_dsd" tr0 m0
            [po; pf; pg1; pg2; pw]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists fe_out,
                Fp12_bounded Fp12_loose fe_out /\
        ...
        (FElem_Fp12 pout old_out ⋆
         (FElem_Fp p_px p_x ⋆
          (FElem_Fp p_py p_y ⋆
           (FElem_Fp2 p_qx q_x ⋆
            (FElem_Fp2 p_qy q_y ⋆ Rr))))) mem ->
        WeakestPrecondition.call functions "bn446_pairing_dsd" tr mem
    ...
    Proof.
      intros functions tr mem pout p_px p_py p_qx p_qy
        ...
        Hbqx Hbqy Hbpx Hbpy Hsep.
      pose proof (bn446_pairing_dsd_ok functions HEnv HLG1 HLG2 HLW HML HFE
