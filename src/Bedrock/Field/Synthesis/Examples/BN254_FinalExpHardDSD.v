(** * BN254 DSD Final Exponentiation Hard Part WP Proof
    Proves bn254_final_exp_hard_dsd (35 calls, 7 stackallocs) satisfies its spec.
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.BN254_PowU.
...
Section BN254_FinalExpHardDSD.
    ...
    Definition spec_of_pow_u_inplace (functions : @map.rep String.string
      ...
      forall pbase (base_val : Fp12_felem) Rr tr mem,
        Fp12_bounded Fp12_tight base_val ->
        (FElem_Fp12 pbase base_val ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn254_Fp12_pow_u"
          tr mem [pbase; pbase]
          (fun tr' mem' rets =>
    ...
    Instance spec_of_bn254_load_gamma1 : spec_of "bn254_load_gamma1" :=
      fnspec! "bn254_load_gamma1" (pout : word)
    ...
    Instance spec_of_bn254_load_gamma2 : spec_of "bn254_load_gamma2" :=
      fnspec! "bn254_load_gamma2" (pout : word)
        / (old_out : Fp2_felem) Rr,
      { requires tr mem := (FElem_Fp2 pout old_out ⋆ Rr) mem;
    ...
    Instance spec_of_bn254_load_w_frob_c1 : spec_of "bn254_load_w_frob_c1" :=
      fnspec! "bn254_load_w_frob_c1" (pout : word)
        / (old_out : Fp2_felem) Rr,
      { requires tr mem := (FElem_Fp2 pout old_out ⋆ Rr) mem;
    ...
    Local Instance bn254_Fp2_rep_ok' :
      @AbstractField.FieldRepresentation_ok _ bn254_Fp2_params' _ _ _ _ bn254_Fp2_rep' :=
      QuadraticFieldExtensionsSpecs.Fp2_field_representation_ok bn254_beta fp2_prefix.

    Lemma bn254_final_exp_hard_dsd_ok :
      forall functions
        (EnvContains : map.get functions "bn254_final_exp_hard_dsd" =
          Some (snd BN254_Pairing.bn254_final_exp_hard_dsd))
        ...
        (HFloadg1 : spec_of_bn254_load_gamma1 functions)
        (HFloadg2 : spec_of_bn254_load_gamma2 functions)
        (HFloadw : spec_of_bn254_load_w_frob_c1 functions),
      spec_of_bn254_final_exp_hard_dsd functions.
    Proof.
      intros. unfold spec_of_bn254_final_exp_hard_dsd.
      ...
      unfold BN254_Pairing.bn254_final_exp_hard_dsd. simpl snd. simpl fst. cbv match beta.
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn254_Fp12_params' _ _ _ _
        bn254_Fp12_rep' wordok mapok a_t0 mS_t0) HaS_t0) as [t0i Ht0i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn254_Fp12_params' _ _ _ _
        bn254_Fp12_rep' wordok mapok a_t1 mS_t1) HaS_t1) as [t1i Ht1i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn254_Fp12_params' _ _ _ _
        bn254_Fp12_rep' wordok mapok a_t2 mS_t2) HaS_t2) as [t2i Ht2i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn254_Fp12_params' _ _ _ _
        bn254_Fp12_rep' wordok mapok a_t3 mS_t3) HaS_t3) as [t3i Ht3i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn254_Fp2_params' _ _ _ _
        bn254_Fp2_rep' wordok mapok a_gamma1 mS_g1) HaS_g1) as [g1i Hg1i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn254_Fp2_params' _ _ _ _
        bn254_Fp2_rep' wordok mapok a_gamma2 mS_g2) HaS_g2) as [g2i Hg2i].
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn254_Fp2_params' _ _ _ _
        bn254_Fp2_rep' wordok mapok a_wfc1 mS_w) HaS_w) as [wi Hwi].

      unfold BN254_Pairing.final_exp_hard_dsd_body, BN254_Pairing.cmd_seq_list.
      ...
      (* ================================================================ *)
      (* Call 1: bn254_load_gamma1(gamma1)                                *)
      (* ================================================================ *)
      repeat straightline.
      ...
      (* ================================================================ *)
      (* Call 2: bn254_load_gamma2(gamma2)                                *)
      (* ================================================================ *)
      repeat straightline.
      eapply Semantics.weaken_call.
      ...
      (* ================================================================ *)
      (* Call 3: bn254_load_w_frob_c1(w_frob_c1)                         *)
      (* ================================================================ *)
      repeat straightline.
      eapply Semantics.weaken_call.
      ...
      { pose proof (AbstractField.FElem_to_bytes
                      (field_representation:=bn254_Fp2_rep')
