(** * BLS12-377 Miller Loop WP Proof
    Standalone WP correctness proof for bls377_miller_loop from BLS12_377_Pairing.v.
    Uses Loops.while_localsmap with a 65->0 nat measure.

    Key differences from BLS12-381 (BLS12_MillerLoop.v):
    - beta = -5, xi = (0, 1)
    - 65 iterations (66-bit 6u+2 parameter) instead of 63
    - u6p2 stored as 2-word array on stack; bit extraction involves a memory load
...
(* ================================================================ *)
(* BLS12-377 Section context -- mirrors BLS12_377_PowU.v             *)
(* ================================================================ *)

Section BLS12_377_MillerLoop.
    ...
    Instance spec_of_Fp2_add : spec_of (AbstractField.add (F:=Fp2)) :=
      AbstractField.binop_spec (F:=Fp2) (field_representation:=bls377_Fp2_rep') AbstractField.bin_add.
    ...
    Instance spec_of_Fp2_sqr : spec_of (AbstractField.square (F:=Fp2)) :=
      AbstractField.unop_spec (F:=Fp2) (field_representation:=bls377_Fp2_rep') AbstractField.un_square.

    Instance spec_of_Fp2_inv : spec_of (AbstractField.inv (F:=Fp2)) :=
      AbstractField.unop_spec (F:=Fp2) (field_representation:=bls377_Fp2_rep') AbstractField.un_inv.
    ...
    (* spec_of for bls377_make_line -- needed by straightline_call *)
    Instance spec_of_bls377_make_line : spec_of "bls377_make_line" :=
      fnspec! "bls377_make_line" (pout plam pxt pyt pxp pyp : word)
    ...
    (* ============================================================ *)
    (* D1: bls377_miller_loop spec and proof                         *)
    (* ============================================================ *)

    Instance spec_of_bls377_miller_loop : spec_of "bls377_miller_loop" :=
      fnspec! "bls377_miller_loop" (pout p_px p_py p_qx p_qy : word)
    ...
    (* u6p2 limbs: 6u+2 = 0x31e34800000000008 stored as [lo, hi] *)
    Local Definition u6p2_limbs : list word :=
      [word.of_Z 0x1e34800000000008; word.of_Z 0x3].
    ...
    Proof.
      ...
      (* Convert each 8-byte array to anybytes *)
      ...
      (* Convert each anybytes 8 to scalar via anybytes_to_scalar *)
    ...
    (* u6p2 store lemma: process 2 stores to stack-allocated u6p2 array.
       ...
       Axiomatized for now — the store WP + anybytes decomposition is mechanical
       but requires careful address arithmetic. *)
    Local Lemma u6p2_stores_wp :
      forall call t (m : mem) l (a_u6p2 : word) R
        ...
        WeakestPrecondition.cmd call
          (BLS12_377_Pairing.store_6u2_limbs) t m l post.
    Proof.
      intros call t m l a_u6p2 R post Hget Hany Hpost.
      (* Get two scalars from the 16-byte anybytes region *)
      pose proof (anybytes16_to_2scalars a_u6p2 R m Hany) as [w0 [w1 Hsep2]].
      (* Unfold store_6u2_limbs into cmd.seq of two stores *)
      unfold BLS12_377_Pairing.store_6u2_limbs, BLS12_377_Pairing.cmd_seq_list.
      ...
      (* Now have sep with scalar a_u6p2 lo * (scalar (a_u6p2+8) hi * R) *)
      (* Convert to array scalar (word.of_Z 8) a_u6p2 [lo; hi] * R *)
      apply Hpost.
    ...
    Lemma bls377_miller_loop_ok :
      forall functions
        (EnvContains : map.get functions "bls377_miller_loop" =
          Some (snd bls377_miller_loop))
        ...
        (HMakeLine : map.get functions "bls377_make_line" =
          Some (snd bls377_make_line))
        (HFp2mulfpEnv : map.get functions "bls377_Fp2_mul_fp" =
          Some (snd bls377_Fp2_mul_fp))
        (HMakeLineOk : spec_of_bls377_make_line functions),
      spec_of_bls377_miller_loop functions.
    Proof.
      intros.
      unfold spec_of_bls377_miller_loop.
      ...
      unfold bls377_miller_loop. simpl snd. simpl fst.
      cbv match beta.
      eexists. split. { exact eq_refl. }
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bls377_Fp12_params' _ _ _ _ bls377_Fp12_rep'
      ...
      (* === Build master separation logic hypothesis ===
         ...
         hypotheses for stackallocs 1-7 have been destructed by
         straightline_cleanup. Only HsplitU6p2 survives (no straightline
         after the last stackalloc). *)
      ...
      unfold BLS12_377_Pairing.miller_loop_full_body.
      unfold BLS12_377_Pairing.cmd_seq_list.
      unfold BLS12_377_Pairing.fp12_set_one.
      ...
      Local Notation fp6_felem_offset :=
        (Memory.bytes_per_word 64 * Z.of_nat (@AbstractField.felem_size_in_words _ bls377_Fp6_params' _ _ _ _ bls377_Fp6_rep')).
      ...
      change (Fp2_field_parameters bls377_beta fp2_prefix)
        with bls377_Fp2_params' in Hfe_c00, Hfe_c01, Hfe_c02, Hfe_c10, Hfe_c11, Hfe_c12.
      change (Fp2_field_representation bls377_beta fp2_prefix)
        with bls377_Fp2_rep' in Hfe_c00, Hfe_c01, Hfe_c02, Hfe_c10, Hfe_c11, Hfe_c12.
      ...
      { unfold miller_loop_inv.
        ...
        { (* Fp12_bounded Fp12_tight f_new -- from 12 from_word tight bounds *)
          ...
          change Fp12_bounded with
            (fun (b : @AbstractField.bounds _ bls377_Fp6_params' _ _ _ _ bls377_Fp6_rep')
                 (felem : list word) =>
              @AbstractField.bounded_by _ bls377_Fp6_params' _ _ _ _ bls377_Fp6_rep' b (d0_felem felem) /\
              @AbstractField.bounded_by _ bls377_Fp6_params' _ _ _ _ bls377_Fp6_rep' b (d1_felem felem));
            cbv beta.
          rewrite (@DodecicFieldExtensions.d0_felem_app _ _ _ _
            bls377_pf_params bls377_Fp_rep bls377_beta bls377_xi_re bls377_xi_im fp6_prefix fp2_prefix
          ...
          rewrite (@DodecicFieldExtensions.d1_felem_app _ _ _ _
            ...
            ((fw7 ++ fw8) ++ (fw9 ++ fw10) ++ (fw11 ++ fw12))
            Hlen_d0_fp6).
          change (@AbstractField.bounded_by _ bls377_Fp6_params' _ _ _ _ bls377_Fp6_rep') with
            (fun (b : @AbstractField.bounds _ bls377_Fp2_params' _ _ _ _ bls377_Fp2_rep')
                 (felem : list word) =>
          ...
          rewrite (@CubicFieldExtensions.c0_felem_app _ _ _ _
            bls377_pf_params bls377_beta bls377_Fp_rep fp2_prefix
          ...
          change Fp2_bounded with
            (fun (b : @AbstractField.bounds _ _ _ _ _ _ bls377_Fp_rep)
                 (ws : list word) =>
      ...
      { intros vi ti mi li Hinv.
        ...
        { (* TRUE branch: word.unsigned br <> 0, loop body *)
          ...
          unfold BLS12_377_Pairing.miller_loop_iteration.
          unfold BLS12_377_Pairing.cmd_seq_list.
          ...
          { (* Evaluate load expression: provide the load result directly *)
            ...
            { cbv [u6p2_limbs]. simpl length.
              ...
              { rewrite word.unsigned_of_Z. change (word.wrap 6) with 6.
                ...
                { assert (Hv_eq : v = word.sub (word.of_Z (Z.of_nat vi)) (word.of_Z 1)).
        ...
        { (* FALSE branch: word.unsigned br = 0, postcondition *)
          ...
          split. { pose proof (@DodecicFieldExtensionsSpecs.Fp12_field_representation_ok
                     _ _ _ _ bls377_pf_params bls377_Fp_rep bls377_Fp_rep_ok bls377_beta
                     bls377_xi_re bls377_xi_im fp12_prefix fp6_prefix fp2_prefix) as Hfp12_ok.
                   exact (@AbstractField.relax_bounds _ _ _ _ _ _ _ Hfp12_ok _ Hbf_vi). }
          exact Hrest_f. }
      }
    Qed.

End BLS12_377_MillerLoop.
