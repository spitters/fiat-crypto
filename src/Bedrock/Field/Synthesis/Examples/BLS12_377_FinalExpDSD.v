(** * BLS12-377 DSD Final Exponentiation WP Proof
    Proves bls377_final_exp_dsd (6 calls) satisfies its spec.
...
Section BLS12_377_FinalExpDSD.
    ...
    Instance spec_of_Fp12_inv : spec_of (AbstractField.inv (F:=Fp12)) :=
      AbstractField.unop_spec (F:=Fp12) (field_representation:=bls377_Fp12_rep') AbstractField.un_inv.
    ...
    Instance spec_of_bls377_final_exp_dsd : spec_of "bls377_final_exp_dsd" :=
      fnspec! "bls377_final_exp_dsd"
    ...
    Lemma bls377_final_exp_dsd_ok :
      forall functions
        (EnvContains : map.get functions "bls377_final_exp_dsd" =
          Some (snd BLS12_377_Pairing.bls377_final_exp_dsd))
        ...
        (HFhard : spec_of_bls377_final_exp_hard_dsd functions),
      spec_of_bls377_final_exp_dsd functions.
    Proof.
      intros. unfold spec_of_bls377_final_exp_dsd.
      ...
      unfold BLS12_377_Pairing.bls377_final_exp_dsd. simpl snd. simpl fst. cbv match beta.
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bls377_Fp12_params' _ _ _ _
        bls377_Fp12_rep' wordok mapok a_result mSr) HaSr) as [ri Hri].

      (* Stackalloc 2: tmp *)
      straightline. split. { apply Z_mod_mult. }
      intros a_tmp mSt mCt HaSt HmSt.
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bls377_Fp12_params' _ _ _ _
        bls377_Fp12_rep' wordok mapok a_tmp mSt) HaSt) as [ti Hti].

      unfold BLS12_377_Pairing.final_exp_dsd_body, BLS12_377_Pairing.cmd_seq_list.
      ...
      (* Hsep_hard :
           (FElem_Fp12 pout hard_out ⋆ (FElem_Fp12 a_result mul2_out ⋆
             (FElem_Fp12 a_tmp frob_out ⋆ (... gammas ... FElem_Fp12 pf f ⋆ Rr)))) m'4

         Goal: exists m' mStack', anybytes a_tmp mStack' /\ split m'4 m' mStack' /\
                 exists m'0 mStack'0, anybytes a_result mStack'0 /\ split m' m'0 mStack'0 /\
                   postcondition m'0 *)
