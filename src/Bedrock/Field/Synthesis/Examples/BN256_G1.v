(** * BN256 G1 group operation: ladderstep (combined doubling + addition).

    The function body is the generic [ladderstep_body] from CurveAdd.v
    instantiated for BN256 (3b = 9). The WP correctness lemma states
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.bn256_three_b.
...
Section bn256_G1.
    ...
    (* BN256 curve: y^2 = x^3 + 3, so 3b = 9 *)
    Definition bn256_G1_add : function_t :=
      ("curve_add", ladderstep_body "bn256_three_b").
    ...
    (* The actual spec parameterized over a bounded three_b witness.
       ...
       which is provided by bn256_three_b.three_b_mont_valid. *)
    Definition bn256_G1_spec_statement
      (three_b : Crypto.Bedrock.Specs.Field.felem)
      (functions : Semantics.env) : Prop :=
      @CurveAdd.spec_of_ladderstep _ _ _ _ _ _
        bn256_field_parameters bn256_frep three_b functions.
