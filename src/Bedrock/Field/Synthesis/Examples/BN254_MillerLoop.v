...
Section BN254_MillerLoop.
    ...
    Instance spec_of_bn254_make_line : spec_of "bn254_make_line" :=
      fnspec! "bn254_make_line" (pout plam pxt pyt pxp pyp : word)
    ...
    Instance spec_of_bn254_miller_loop : spec_of "bn254_miller_loop" :=
      fnspec! "bn254_miller_loop" (pout p_px p_py p_qx p_qy : word)
    ...
    Lemma bn254_miller_loop_ok :
      forall functions
        (EnvContains : map.get functions "bn254_miller_loop" =
          Some (snd bn254_miller_loop))
        ...
        (HMakeLine : map.get functions "bn254_make_line" =
          Some (snd bn254_make_line))
        (HFp2mulfpEnv : map.get functions "bn254_Fp2_mul_fp" =
          Some (snd bn254_Fp2_mul_fp))
        (HMakeLineOk : spec_of_bn254_make_line functions),
      spec_of_bn254_miller_loop functions.
    Proof.
      intros.
      unfold spec_of_bn254_miller_loop.
      ...
      unfold bn254_miller_loop. simpl snd. simpl fst.
      cbv match beta.
      ...
      (* Stackalloc 8: u6p2 (8 bytes = 1 word) *)
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bn254_Fp12_params' _ _ _ _ bn254_Fp12_rep'
      ...
      unfold BN254_Pairing.miller_loop_full_body.
      unfold BN254_Pairing.cmd_seq_list.
      unfold BN254_Pairing.fp12_set_one.
      ...
      change (Fp2_field_parameters bn254_beta fp2_prefix)
        with bn254_Fp2_params' in Hfe_c00, Hfe_c01, Hfe_c02, Hfe_c10, Hfe_c11, Hfe_c12.
      change (Fp2_field_representation bn254_beta fp2_prefix)
        with bn254_Fp2_rep' in Hfe_c00, Hfe_c01, Hfe_c02, Hfe_c10, Hfe_c11, Hfe_c12.
      ...
      { unfold miller_loop_inv.
        ...
        { (* Fp12_bounded Fp12_tight f_new -- from 12 from_word tight bounds *)
          ...
          rewrite (@CubicFieldExtensions.c0_felem_app _ _ _ _
            bn254_pf_params bn254_beta bn254_Fp_rep fp2_prefix
      ...
      { intros vi ti mi li Hinv.
        ...
        { (* TRUE branch: word.unsigned br <> 0, loop body *)
          ...
          unfold BN254_Pairing.miller_loop_iteration.
          unfold BN254_Pairing.cmd_seq_list.
        ...
        { (* FALSE branch: word.unsigned br = 0, postcondition *)
          ...
          (* --- Dealloc level 1: u6p2 (8 bytes) --- *)
          eassert (Hu6p2_sep : (_ ⋆ scalar a_u6p2 u6p2_word) m_cp).
          ...
          split. { exact (scalar_to_anybytes8 _ _ _ Hu6p2). }
