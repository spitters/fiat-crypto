...
Section BLS12_PowX.
    ...
    (* === Abbreviations (matching BLS12_FinalExp.v exactly) === *)
    ...
    (* BLS parameter |x| *)
    Local Definition bls_x_abs : Z := 0xd201000000010000.

    (* ==================================================================== *)
    (* Loop invariant for pow_x                                             *)
    (* ==================================================================== *)

    Definition pow_x_inv
    ...
    (* ==================================================================== *)
    (* Main WP theorem (skeleton — proof body to be filled in)              *)
    (* ==================================================================== *)
    ...
    Proof.
      ...
      { pose proof (@AbstractField.FElem_from_bytes _ bls12_Fp12_params' _ _ _ _ bls12_Fp12_rep') as Hconv.
      ...
      eapply Loops.while_localsmap
        with (v0 := 63%nat) (lt := Nat.lt)
             (invariant := fun v t m l =>
                pow_x_inv a_result pbase pout mem0 (FElem_Fp12 pout tr ⋆ old_out) t' v t m l).
      { exact lt_wf. }
      { (* Initial invariant *)
        unfold pow_x_inv. split. { reflexivity. } split. { lia. }
        exists mem0. split. { exact Hbbase. }
        split. { exact Hsep_copy. }
        subst l0 l i. solve_mapgets_n. }
      { (* Loop body + exit *)
        intros v t_v m_v l_v Hinv.
        unfold pow_x_inv in Hinv.
        ...
        { (* TRUE branch: v ≠ 0, process loop body *)
          intro Hne.
          unfold BLS12_Pairing.pow_x_loop_body, BLS12_Pairing.cmd_seq_list.
          (* i = i - 1 *)
          eexists. split.
          ...
          set (bit_val := word.and (word.sru (word.of_Z 15132376222941642752)
        ...
        { (* FALSE branch: v = 0, exit loop *)
          intro Heq.
          unfold BLS12_Pairing.cmd_seq_list. repeat straightline.
          ...
          cbv [list_map get].
          refine (conj eq_refl (ex_intro _ result_v (conj Hbr _))).
