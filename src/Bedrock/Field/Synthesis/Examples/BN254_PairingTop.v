(** * BN254 Top-Level Pairing WP Proof
    Standalone WP correctness proof for bn254_pairing_dsd from BN254_Pairing.v,
    plus the top-level bn254_pairing_dsd_correct_standalone theorem.
*)
...
Section BN254_PairingTop.
    ...
    Instance spec_of_bn254_pairing_dsd : spec_of "bn254_pairing_dsd" :=
      fnspec! "bn254_pairing_dsd" (pout p_px p_py p_qx p_qy : word)
    ...
    Lemma bn254_pairing_dsd_ok :
      forall functions
        (EnvContains : map.get functions "bn254_pairing_dsd" =
          Some (snd bn254_pairing_dsd))
        ...
        (HMillerLoop : forall ptmp ppx ppy pqx pqy
          ...
          Semantics.call functions "bn254_miller_loop" tr m
                ...
                (FElem_Fp12 po fe_out ⋆
                 (FElem_Fp12 pf f ⋆
                  (FElem_Fp2 pg1 g1 ⋆
                   (FElem_Fp2 pg2 g2 ⋆
                    (FElem_Fp2 pw w ⋆ R))))) m')),
      spec_of_bn254_pairing_dsd functions.
    Proof.
      intros.
      unfold spec_of_bn254_pairing_dsd.
      ...
      unfold bn254_pairing_dsd. simpl snd. simpl fst.
      cbv match beta.
      ...
      unfold BN254_Pairing.pairing_dsd_body, BN254_Pairing.cmd_seq_list.
    ...
    Theorem bn254_pairing_dsd_correct_standalone :
      forall functions tr mem pout p_px p_py p_qx p_qy
        (old_out : Fp12_felem) (p_x p_y : Fp_felem)
        (q_x q_y : Fp2_felem) Rr,
        map.get functions "bn254_pairing_dsd" = Some (snd bn254_pairing_dsd) ->
        (* Loader hypotheses *)
        ...
        (forall ptmp ppx ppy pqx pqy
          ...
          Semantics.call functions "bn254_miller_loop" tr0 m0
            [ptmp; ppx; ppy; pqx; pqy]
            (fun tr' m' rets => rets = [] /\ tr0 = tr' /\
              exists ml_out,
                Fp12_bounded Fp12_tight ml_out /\
        ...
        (FElem_Fp12 pout old_out ⋆
         (FElem_Fp p_px p_x ⋆
          (FElem_Fp p_py p_y ⋆
           (FElem_Fp2 p_qx q_x ⋆
            (FElem_Fp2 p_qy q_y ⋆ Rr))))) mem ->
        WeakestPrecondition.call functions "bn254_pairing_dsd" tr mem
    ...
    Proof.
      intros functions tr mem pout p_px p_py p_qx p_qy
        ...
        Hbqx Hbqy Hbpx Hbpy Hsep.
      pose proof (bn254_pairing_dsd_ok functions HEnv HLG1 HLG2 HLW HML HFE
