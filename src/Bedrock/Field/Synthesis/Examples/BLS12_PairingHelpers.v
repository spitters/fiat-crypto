(** * BLS12-381 Pairing Helper WP Proofs
    Standalone WP correctness proofs for pairing helper functions
    defined in BLS12_Pairing.v:
    - C1: bls12_Fp2_mul_fp (multiply Fp2 by Fp scalar)
    - C3: bls12_load_gamma1_p2
    - C4: bls12_load_gamma2_p2
    - C5: bls12_load_w_frob_p2_c1
    - C2: bls12_make_line
*)
...
(* ================================================================ *)
(* BLS12-381 Section context — mirrors BLS12_Pairing.v              *)
(* ================================================================ *)

Section BLS12_PairingHelpers.
    ...
    (* The function body defined here (via bls12_pf_params) must equal
       the imported one from BLS12_Pairing (via bls12_prime_params).
       Both define M_pos = Z.to_pos bls12_prime.m and the same function names,
    ...
    Instance spec_of_Fp_opp : spec_of (@AbstractField.opp _ prime_field_parameters) :=
      AbstractField.unop_spec (F:=Fp) (field_parameters:=prime_field_parameters)
        (field_representation:=bls12_Fp_rep) AbstractField.un_opp.
    ...
    Lemma FElem_Fp_join_in_sep p (a b : Fp_felem) R m :
      length a = @AbstractField.felem_size_in_words _ _ _ _ _ _ bls12_Fp_rep ->
      length b = @AbstractField.felem_size_in_words _ _ _ _ _ _ bls12_Fp_rep ->
      (FElem_Fp p a ⋆
    ...
    Instance spec_of_bls12_Fp2_mul_fp : spec_of "bls12_Fp2_mul_fp" :=
      fnspec! "bls12_Fp2_mul_fp" (pout px ps : word)
    ...
    Lemma bls12_Fp2_mul_fp_ok :
      forall functions
        (EnvContains : map.get functions "bls12_Fp2_mul_fp" =
          Some (snd bls12_Fp2_mul_fp))
        (HFmul : spec_of_Fp_mul functions),
      spec_of_bls12_Fp2_mul_fp functions.
    Proof.
      intros.
      unfold spec_of_bls12_Fp2_mul_fp.
      ...
      unfold bls12_Fp2_mul_fp. simpl snd. simpl fst.
      cbv match beta.
      eexists. split. { exact eq_refl. }
      (* Step through: cmd.seq -> cmd.call arg evaluation *)
      straightline. straightline. straightline.
      (* Now at: call functions "bls12_mul" tr mem0 args postcondition *)
      ...
      change (@AbstractField.bounded_by _ bls12_Fp2_params' _ _ _ _ bls12_Fp2_rep')
      ...
      (* === Call 1: bls12_mul(out, x, s) — fst halves === *)
      ...
      1: { pose proof HFmul as HFmul1.
           ...
           split; [apply (@AbstractField.relax_bounds _ _ _ _ _ _
             bls12_Fp_rep bls12_Fp_rep_ok); exact Hbx1 |].
           split; [exact Hbs |].
      ...
      (* === Call 2: bls12_mul(out+off, x+off, s) — snd halves === *)
      eapply Semantics.weaken_call.
      1: { pose proof HFmul as HFmul2.
           ...
           split; [apply (@AbstractField.relax_bounds _ _ _ _ _ _
             bls12_Fp_rep bls12_Fp_rep_ok); exact Hbx2 |].
           split; [exact Hbs |].
      ...
      { destruct Hsep1 as [mc [_ [_ [Hfc _]]]].
        exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bls12_pf_params bls12_Fp_rep _ _ _ Hfc). }
      ...
      assert (Hlen_x1 : length (fst_felem x) =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bls12_Fp_rep).
      { exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bls12_pf_params bls12_Fp_rep _ _ _ Hx1). }
      assert (Hlen_x2 : length (snd_felem x) =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bls12_Fp_rep).
      { exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bls12_pf_params bls12_Fp_rep _ _ _ Hx2). }
      ...
      { (* Decompose Hsep2 to get individual maps *)
        ...
        rewrite (@QuadraticFieldExtensions.Fp2_list_decomp _ _ _ _
          bls12_pf_params bls12_Fp_rep x) in Hfp2_x.
        (* Build final sep *)
    ...
    Local Ltac solve_store_fp2_constant :=
      ...
      [ (* Sep: prove via Bignum_of_array_sep to avoid emp issues *)
        ...
        change (@AbstractField.felem_size_in_words
          _ bls12_Fp2_params' _ _ _ _ bls12_Fp2_rep') with 12%nat;
    ...
    Lemma bls12_load_gamma1_p2_ok :
      forall functions
        (EnvContains : map.get functions "bls12_load_gamma1_p2" =
          Some (snd bls12_load_gamma1_p2)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bls12_load_gamma1_p2" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bls12_load_gamma1_p2. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
    ...
    Lemma bls12_load_gamma2_p2_ok :
      forall functions
        (EnvContains : map.get functions "bls12_load_gamma2_p2" =
          Some (snd bls12_load_gamma2_p2)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bls12_load_gamma2_p2" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bls12_load_gamma2_p2. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
      solve_store_fp2_constant.
    Qed.
    ...
    Lemma bls12_load_w_frob_p2_c1_ok :
      forall functions
        (EnvContains : map.get functions "bls12_load_w_frob_p2_c1" =
          Some (snd bls12_load_w_frob_p2_c1)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bls12_load_w_frob_p2_c1" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bls12_load_w_frob_p2_c1. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
      solve_store_fp2_constant.
    Qed.

    (* ============================================================ *)
    (* C2: bls12_make_line (Admitted stub)                          *)
    (* ============================================================ *)

    (* Fp6-level offset notations (from CubicFieldExtensions) *)
    Local Notation fp2_felem_offset :=
      (Memory.bytes_per_word 64 * Z.of_nat (@AbstractField.felem_size_in_words _ bls12_Fp2_params' _ _ _ _ bls12_Fp2_rep')).
    Local Notation Fp6_felem_size := (@AbstractField.felem_size_in_words _ bls12_Fp6_params' _ _ _ _ bls12_Fp6_rep').
    ...
    Lemma bls12_make_line_ok :
      forall functions
        (EnvContains : map.get functions "bls12_make_line" =
        ...
        (HFp2mulfp : spec_of_bls12_Fp2_mul_fp functions)
        (HFpcopy : spec_of_Fp_felem_copy functions)
        (HFfromword : spec_of_Fp_from_word functions),
      forall pout plam pxt pyt pxp pyp
        (old_out : @AbstractField.felem _ bls12_Fp12_params' _ _ _ _ bls12_Fp12_rep')
        ...
        WeakestPrecondition.call functions "bls12_make_line" tr mem
          [pout; plam; pxt; pyt; pxp; pyp]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              @AbstractField.bounded_by _ bls12_Fp12_params' _ _ _ _ bls12_Fp12_rep'
                (@AbstractField.loose_bounds _ bls12_Fp12_params' _ _ _ _ bls12_Fp12_rep') out /\
              (FElem_Fp12 pout out ⋆
               (FElem_Fp2 plam lam ⋆
                (FElem_Fp2 pxt xt ⋆
                 (FElem_Fp2 pyt yt ⋆
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bls12_make_line. simpl snd. simpl fst.
      cbv match beta.
      eexists. split. { exact eq_refl. }
      repeat straightline.
      ...
      1: { pose proof HFp2mul as HF1.
           ...
           split; [apply (@AbstractField.relax_bounds _ bls12_Fp2_params' _ _ _ _
             bls12_Fp2_rep' Fp2_rep_ok); exact Hblam |].
           split; [apply (@AbstractField.relax_bounds _ bls12_Fp2_params' _ _ _ _
             bls12_Fp2_rep' Fp2_rep_ok); exact Hbxt |].
      ...
      1: { pose proof HFp2mulfp as HF3.
           unfold spec_of_bls12_Fp2_mul_fp in HF3.
      ...
      *)
      ...
      { (* bounded_by loose_bounds the_out *)
        ...
        unfold Fp2_bounded, AbstractField.bounded_by,
               bls12_Fp2_rep', bls12_Fp2_params',
        ...
        pose proof (@AbstractField.relax_bounds _ _ _ _ _ _
          bls12_Fp_rep bls12_Fp_rep_ok) as RB.
        ...
        Local Typeclasses Transparent bls12_Fp12_params'.
        Local Typeclasses Transparent bls12_Fp12_rep'.
        Local Typeclasses Transparent bls12_Fp6_params'.
        Local Typeclasses Transparent bls12_Fp6_rep'.
        Local Typeclasses Transparent bls12_Fp2_params'.
        Local Typeclasses Transparent bls12_Fp2_rep'.
        cbv [AbstractField.bounded_by AbstractField.loose_bounds
             bls12_Fp12_rep' bls12_Fp12_params'
             bls12_Fp6_rep' bls12_Fp6_params'
             bls12_Fp2_rep' bls12_Fp2_params'
             bls12_beta bls12_xi_re bls12_xi_im
        ...
        split.
        (* d0 bound *)
        { change (DodecicFieldExtensionsSpecs.Fp6_repr_inst bls12_beta bls12_xi_re bls12_xi_im) with bls12_Fp6_rep'.
          ...
          cbv [DodecicFieldExtensionsSpecs.Fp6_repr_inst bls12_beta bls12_xi_re bls12_xi_im
               CubicFieldExtensionsSpecs.Fp6_field_representation
               CubicFieldExtensionsSpecs.Fp6_field_parameters
               CubicFieldExtensionsSpecs.c0_felem
               CubicFieldExtensionsSpecs.c1_felem
