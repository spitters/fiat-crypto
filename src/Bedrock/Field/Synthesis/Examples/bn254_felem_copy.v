...
Section FelemCopy.
    ...
    Existing Instance bn254_field_parameters.
    Existing Instance bn254_frep.
    Existing Instance bn254_frep_ok.

    (* BN254 uses 4 x 64-bit words (254 bits) *)
    Definition bn254_felem_copy : Syntax.func := (["out"; "in"], (nil : list string), bedrock_func_body:(
    ...
    Instance bn254_felem_copy_spec : spec_of felem_copy := spec_of_felem_copy.

    Lemma felem_copy_ok : program_logic_goal_for_function! bn254_felem_copy.
    Proof.
      ...
      cbv match beta delta [WeakestPrecondition.func bn254_felem_copy].
