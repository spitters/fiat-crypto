...
Section FelemCopy.
    ...
    Existing Instance bls377_field_parameters.
    Existing Instance bls377_frep.
    Existing Instance bls377_frep_ok.

    (* BLS12-377 also uses 6 × 64-bit words (377 bits), same body as BLS12-381 *)
    Definition bls377_felem_copy : Syntax.func := (["out"; "in"], (nil : list string), bedrock_func_body:(
    ...
    Instance bls377_felem_copy_spec : spec_of felem_copy := spec_of_felem_copy.

    Lemma felem_copy_ok : program_logic_goal_for_function! bls377_felem_copy.
    Proof.
      ...
      cbv match beta delta [WeakestPrecondition.func bls377_felem_copy].
