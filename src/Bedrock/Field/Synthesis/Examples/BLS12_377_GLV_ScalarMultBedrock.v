(** * BLS12-377 GLV Shamir scalar multiplication -- bedrock2 implementation.

    Implements Shamir's trick for the GLV two-scalar multiplication:
      [k]P = [k1]P + [k2]phi(P)
    where (k1, k2) = glv_decompose(k), with k1, k2 each at most 128 bits.
    ...
    The loop runs 128 iterations (MSB to LSB via shift-and-test),
...
Section GLV_Shamir_Generic.
  ...
  (* Gallina Shamir double-and-add (MSB-first, 128 iterations).
  ...
  *)
    ...
    ("bls12_377_glv_shamir",
     (["outx"; "outy"; "outz";
      ...
      bedrock_func_body:(
        ...
        (* Main loop: 128 iterations (bits 0..128) *)
  ...
  Instance spec_of_glv_shamir : spec_of "bls12_377_glv_shamir" :=
    fnspec! "bls12_377_glv_shamir"
          ...
          /\ exists Outxnew Outynew Outznew
             ...
             exists k1new k2new : list word,
               (* The output is the Shamir double-scalar multiplication.
                  ...
                  After 128 iterations with k1, k2 < 2^128:
  ...
  (** The LSB-first Shamir loop invariant.
      ...
      After [iter] iterations (iter = 0, 1, ..., 128):
  ...
  Lemma glv_shamir_ok :
    forall functions
      (EnvContains : map.get functions "bls12_377_glv_shamir" =
  ...
  Proof.
    ...
    (* === Phase 3: Process store_zero initialization ===
       ...
       Initial invariant (v0=128, iter=0):
       ...
       === Phase 6: Post-loop ===
       After 128 iterations: out = [k1]P + [k2]phi.
    ...
    { unfold glv_loop_inv.
      ...
      (* iter word value *)
      all: try (change (Z.of_nat 128) with 128%Z;
    ...
    { intros vi ti mi li Hinv.
      ...
      (* Evaluate branch condition:
         expr.op bopname.ltu
           (expr.load access_size.word (expr.var "iter"))
           (expr.literal glv_iterations)
         This loads the iter counter from memory, compares with 128.
         Result is word.b2w (word.ltu iter_word (word.of_Z 128)). *)
      ...
      (* The branch value is computed from memory (load from a_iter).
         ...
         From Hiter_val: word.unsigned iw_i = 128 - Z.of_nat vi *)

      (* Provide branch value: word encoding of (iter < 128) *)
      ...
      { (* Evaluate branch expression:
           ...
           The load reads iw_i from scalar a_iter, then compares with 128. *)
      ...
      { (* TRUE branch: iter < 128, i.e. vi > 0 *)
        ...
        { rewrite (@word.unsigned_b2w _ _ word_ok) in Hne.
          ...
          destruct (width_cases) as [Hw|Hw]; rewrite Hw in Hltu;
            (change (128 mod 2 ^ 32) with 128 in Hltu
             || change (128 mod 2 ^ 64) with 128 in Hltu); lia. }
        split.
        2: { lia. }
        ...
        all: try (
          transitivity (Z.shiftr (eval glv_scalar_words k1_words) ((128 - Z.of_nat vi) + 1));
          [rewrite <- H0, Hk1_i;
           apply (shiftr_from_div2 _ _ (128 - Z.of_nat vi));
           [pose proof (word.unsigned_range iw_i); lia | reflexivity]
          | replace ((128 - Z.of_nat vi) + 1) with (128 - (Z.of_nat vi - 1)) by lia;
            reflexivity]).
        all: try (
          transitivity (Z.shiftr (eval glv_scalar_words k2_words) ((128 - Z.of_nat vi) + 1));
          [rewrite <- H4, Hk2_i;
           apply (shiftr_from_div2 _ _ (128 - Z.of_nat vi));
           [pose proof (word.unsigned_range iw_i); lia | reflexivity]
          | replace ((128 - Z.of_nat vi) + 1) with (128 - (Z.of_nat vi - 1)) by lia;
        ...
        all: try (
          ...
          replace (128 - (128 - Z.of_nat (vi - 1))) with (Z.of_nat (vi - 1)) by
            (pose proof (word.unsigned_range iw_i); lia);
        ...
        replace (128 - (Z.of_nat vi - 1)) with (iter + 1) by (unfold iter; lia).
      ...
      { (* FALSE branch: iter >= 128, i.e. vi = 0 *)
        intro Hcond.
        (* The while condition is false: NOT (iter < 128), so iter >= 128.
           From Hiter_val: word.unsigned iw_i = 128 - Z.of_nat vi.
           From Hcond: word.unsigned (word.b2w (word.ltu ...)) = 0.
           This means NOT (iw_i < 128), so iw_i >= 128.
           Combined with iter = 128 - Z.of_nat vi, we get vi = 0.
        ...
        { (* Accumulator equation: out = [k1]P + [k2]phi(P) *)
          ...
          destruct (width_cases) as [Hw|Hw]; rewrite Hw in Hltu_eq;
            (change (128 mod 2 ^ 32) with 128 in Hltu_eq
             || change (128 mod 2 ^ 64) with 128 in Hltu_eq);
            assert (Hvi0 : vi = 0%nat) by (pose proof Znat.Nat2Z.is_nonneg vi; lia);
            subst vi;
            change (Z.of_nat 0) with 0 in Hout_i;
            change (128 - 0) with 128 in Hout_i;
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.BN_StraightlineFast.
    ...
    Definition bls12_377_glv_shamir :=
      @glv_shamir_func _ _ _ _ _ _ _ _ _ _
        bls377_field_parameters bls377_frep
        "group_cmov_alt" "store_zero"
        "curve_add" "bls12_377_three_b".
    ...
    1. Reducing k mod r (via BLS12_377_ScalarReduce)
    2. Decomposing k into (k1, k2) via glv_decompose
    3. Computing phi(P) = (omega * P_x, P_y, P_z)
    4. Calling bls12_377_glv_shamir with the precomputed inputs
*)
