(** * BN446 DSD Final Exponentiation WP Proof
    Proves bn446_final_exp_dsd (6 calls) satisfies its spec.
...
Section BN446_FinalExpDSD.
    ...
    Instance spec_of_Fp12_inv : spec_of (AbstractField.inv (F:=Fp12)) :=
      AbstractField.unop_spec (F:=Fp12) (field_representation:=bn446_Fp12_rep') AbstractField.un_inv.
    ...
    Instance spec_of_bn446_final_exp_dsd : spec_of "bn446_final_exp_dsd" :=
      fnspec! "bn446_final_exp_dsd"
    ...
    Lemma bn446_final_exp_dsd_ok :
      forall functions
        (EnvContains : map.get functions "bn446_final_exp_dsd" =
          Some (snd BN446_Pairing.bn446_final_exp_dsd))
        ...
        (HFhard : spec_of_bn446_final_exp_hard_dsd functions),
      spec_of_bn446_final_exp_dsd functions.
    Proof.
      intros. unfold spec_of_bn446_final_exp_dsd.
      ...
      unfold BN446_Pairing.bn446_final_exp_dsd. simpl snd. simpl fst. cbv match beta.
      ...
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp12_params' _ _ _ _
        bn446_Fp12_rep' wordok mapok a_result mSr) HaSr) as [ri Hri].

      (* Stackalloc 2: tmp *)
      straightline. split. { apply Z_mod_mult. }
      intros a_tmp mSt mCt HaSt HmSt.
      pose proof (proj1 (@AbstractField.FElem_from_bytes _ bn446_Fp12_params' _ _ _ _
        bn446_Fp12_rep' wordok mapok a_tmp mSt) HaSt) as [ti Hti].

      unfold BN446_Pairing.final_exp_dsd_body, BN446_Pairing.cmd_seq_list.
      ...
      (* Call 1: fp12_conjugate(result, f) *)
      repeat straightline.
      ...
      (* Call 3: fp12_mul(result, result, tmp) *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: { eapply HFp12mul.
      ...
      (* Call 5: fp12_mul(result, tmp, result) *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: { eapply HFp12mul.
      ...
      (* Call 6: final_exp_hard_dsd(out, result) *)
      repeat straightline.
      eapply Semantics.weaken_call.
      1: { eapply HFhard.
