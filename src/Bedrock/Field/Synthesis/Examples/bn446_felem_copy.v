...
Section FelemCopy.
    ...
    Existing Instance bn446_field_parameters.
    Existing Instance bn446_frep.
    Existing Instance bn446_frep_ok.

    (* BN446 uses 7 x 64-bit words (446 bits) *)
    Definition bn446_felem_copy : Syntax.func := (["out"; "in"], (nil : list string), bedrock_func_body:(
    ...
    Instance bn446_felem_copy_spec : spec_of felem_copy := spec_of_felem_copy.

    Lemma felem_copy_ok : program_logic_goal_for_function! bn446_felem_copy.
    Proof.
      ...
      change felem_size_in_words with 7%nat in xpf.
      destruct xl as [| x0 [| x1 [| x2 [| x3 [| x4 [| x5 [| x6 []]]]]]]];
        try discriminate.
      assert (Hlen56 : Datatypes.length out = Pos.to_nat 56)
        by (change (Z.to_nat felem_size_in_bytes) with (Pos.to_nat 56) in Hlen;
            exact Hlen).
      rewrite Hlen56 in Hsep; rewrite Nat.eqb_refl in Hsep.
      set (ys := bs2ws (Pos.to_nat 8) out) in Hsep.
      assert (Hyslen: length ys = 7%nat)
        by (subst ys;
            change (Pos.to_nat 8) with (Z.to_nat (Memory.bytes_per_word 64));
            change 7%nat with felem_size_in_words;
            apply bs2ws_felem_length; exact Hlen).
      destruct ys as [| y0 [| y1 [| y2 [| y3 [| y4 [| y5 [| y6 []]]]]]]];
        try discriminate.
      clear Hyslen.
      cbn [array] in Hsep.
      change (Memory.bytes_per_word 64) with 8 in Hsep.
      ...
      cbv match beta delta [WeakestPrecondition.func bn446_felem_copy].
