(** * BN256 Miller Loop WP Proof
    Standalone WP correctness proof for bn256_miller_loop from BN256_Pairing.v.
...
(* ================================================================ *)
(* BN256 Section context                                             *)
(* ================================================================ *)

Section BN256_MillerLoop.
    ...
    Instance spec_of_Fp2_add : spec_of (AbstractField.add (F:=Fp2)) :=
      AbstractField.binop_spec (F:=Fp2) (field_representation:=bn256_Fp2_rep') AbstractField.bin_add.
    ...
    Instance spec_of_Fp2_sqr : spec_of (AbstractField.square (F:=Fp2)) :=
      AbstractField.unop_spec (F:=Fp2) (field_representation:=bn256_Fp2_rep') AbstractField.un_square.

    Instance spec_of_Fp2_inv : spec_of (AbstractField.inv (F:=Fp2)) :=
      AbstractField.unop_spec (F:=Fp2) (field_representation:=bn256_Fp2_rep') AbstractField.un_inv.
    ...
    (* spec_of for bn256_make_line -- needed by straightline_call *)
    Instance spec_of_bn256_make_line : spec_of "bn256_make_line" :=
      fnspec! "bn256_make_line" (pout plam pxt pyt pxp pyp : word)
    ...
    (* ============================================================ *)
    (* D1: bn256_miller_loop spec and proof                          *)
    (* ============================================================ *)

    Instance spec_of_bn256_miller_loop : spec_of "bn256_miller_loop" :=
      fnspec! "bn256_miller_loop" (pout p_px p_py p_qx p_qy : word)
    ...
    (* u6p2 value: |6u+2| for BN256 is 66 bits, stored as [lo, hi].
    ...
    Local Definition u6p2_limbs : list word :=
      [word.of_Z BN256_Pairing.bn256_6u2_lo; word.of_Z BN256_Pairing.bn256_6u2_hi].
    ...
    Lemma bn256_miller_loop_ok :
      forall functions
        (EnvContains : map.get functions "bn256_miller_loop" =
          Some (snd bn256_miller_loop))
        ...
        (HMakeLine : map.get functions "bn256_make_line" =
          Some (snd bn256_make_line))
        (HFp2mulfpEnv : map.get functions "bn256_Fp2_mul_fp" =
          Some (snd bn256_Fp2_mul_fp))
        (HMakeLineOk : spec_of_bn256_make_line functions),
      spec_of_bn256_miller_loop functions.
    Proof.
      intros.
      unfold spec_of_bn256_miller_loop.
      ...
      unfold bn256_miller_loop. simpl snd. simpl fst.
      cbv match beta.
      ...
      pose proof (@AbstractField.FElem_from_bytes _ bn256_Fp12_params' _ _ _ _ bn256_Fp12_rep'
      ...
      unfold BN256_Pairing.miller_loop_full_body.
      unfold BN256_Pairing.cmd_seq_list.
      unfold BN256_Pairing.fp12_set_one.
      ...
      Local Notation fp6_felem_offset :=
        (Memory.bytes_per_word 64 * Z.of_nat (@AbstractField.felem_size_in_words _ bn256_Fp6_params' _ _ _ _ bn256_Fp6_rep')).
      ...
      change (Fp2_field_parameters bn256_beta fp2_prefix)
        with bn256_Fp2_params' in Hfe_c00, Hfe_c01, Hfe_c02, Hfe_c10, Hfe_c11, Hfe_c12.
      change (Fp2_field_representation bn256_beta fp2_prefix)
        with bn256_Fp2_rep' in Hfe_c00, Hfe_c01, Hfe_c02, Hfe_c10, Hfe_c11, Hfe_c12.
      ...
      { unfold miller_loop_inv.
        ...
        { (* Fp12_bounded Fp12_tight f_new -- from 12 from_word tight bounds *)
          ...
          change Fp12_bounded with
            (fun (b : @AbstractField.bounds _ bn256_Fp6_params' _ _ _ _ bn256_Fp6_rep')
                 (felem : list word) =>
              @AbstractField.bounded_by _ bn256_Fp6_params' _ _ _ _ bn256_Fp6_rep' b (d0_felem felem) /\
              @AbstractField.bounded_by _ bn256_Fp6_params' _ _ _ _ bn256_Fp6_rep' b (d1_felem felem));
            cbv beta.
          rewrite (@DodecicFieldExtensions.d0_felem_app _ _ _ _
            bn256_pf_params bn256_Fp_rep bn256_beta bn256_xi_re bn256_xi_im fp6_prefix fp2_prefix
          ...
          rewrite (@DodecicFieldExtensions.d1_felem_app _ _ _ _
            ...
            ((fw7 ++ fw8) ++ (fw9 ++ fw10) ++ (fw11 ++ fw12))
            Hlen_d0_fp6).
          change (@AbstractField.bounded_by _ bn256_Fp6_params' _ _ _ _ bn256_Fp6_rep') with
            (fun (b : @AbstractField.bounds _ bn256_Fp2_params' _ _ _ _ bn256_Fp2_rep')
                 (felem : list word) =>
          ...
          rewrite (@CubicFieldExtensions.c0_felem_app _ _ _ _
            bn256_pf_params bn256_beta bn256_Fp_rep fp2_prefix
          ...
          change Fp2_bounded with
            (fun (b : @AbstractField.bounds _ _ _ _ _ _ bn256_Fp_rep)
                 (ws : list word) =>
      ...
      { intros vi ti mi li Hinv.
        ...
        { (* TRUE branch: word.unsigned br <> 0, loop body *)
          ...
          unfold BN256_Pairing.miller_loop_iteration.
          unfold BN256_Pairing.cmd_seq_list.
        ...
        { (* FALSE branch: word.unsigned br = 0, postcondition *)
          ...
          split. { pose proof (@DodecicFieldExtensionsSpecs.Fp12_field_representation_ok
                     _ _ _ _ bn256_pf_params bn256_Fp_rep bn256_Fp_rep_ok bn256_beta
                     bn256_xi_re bn256_xi_im fp12_prefix fp6_prefix fp2_prefix) as Hfp12_ok.
                   exact (@AbstractField.relax_bounds _ _ _ _ _ _ _ Hfp12_ok _ Hbf_vi). }
          exact Hrest_f. }
      }
    Qed.

End BN256_MillerLoop.
