(** * BN256 field tower instances (Fp2 -> Fp6 -> Fp12).

    Provides FieldParameters, FieldRepresentation, and spec_of instances
    for BN256 with beta=-1, xi=(3,1).
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.bn256_Fp2.
...
Section BN256.
  ...
  Existing Instances
    ...
    bn256_prime_parameters bn256_prime_parameters_ok
    bn256_field_representation bn256_field_representation_ok.
  ...
  Definition bn256_beta : F PrimeField.M_pos := F.of_Z PrimeField.M_pos (-1).
  Definition bn256_xi_re : F PrimeField.M_pos := F.of_Z PrimeField.M_pos 3.
  Definition bn256_xi_im : F PrimeField.M_pos := @F.one PrimeField.M_pos.
  ...
  Instance bn256_Fp2_params : AbstractField.FieldParameters (F PrimeField.M_pos * F PrimeField.M_pos) :=
    Fp2_field_parameters bn256_beta fp2_prefix.

  Instance bn256_Fp2_rep : AbstractField.FieldRepresentation (F:=F PrimeField.M_pos * F PrimeField.M_pos) :=
    Fp2_field_representation bn256_beta fp2_prefix.

  Instance bn256_Fp2_rep_ok : AbstractField.FieldRepresentation_ok (F:=F PrimeField.M_pos * F PrimeField.M_pos) :=
    Fp2_field_representation_ok bn256_beta fp2_prefix.

  Instance bn256_Fp2_names : FieldNames (F:=F PrimeField.M_pos * F PrimeField.M_pos) :=
    field_names_prefixed fp2_prefix.

  (* spec_of instances for Fp-level operations *)
  Instance spec_of_bn256_add : spec_of PrimeField.add :=
    AbstractField.binop_spec AbstractField.bin_add (F:=F PrimeField.M_pos).
  Instance spec_of_bn256_sub : spec_of PrimeField.sub :=
    AbstractField.binop_spec AbstractField.bin_sub (F:=F PrimeField.M_pos).
  Instance spec_of_bn256_mul : spec_of PrimeField.mul :=
    AbstractField.binop_spec AbstractField.bin_mul (F:=F PrimeField.M_pos).
  Instance spec_of_bn256_sqr : spec_of PrimeField.square :=
    AbstractField.unop_spec AbstractField.un_square (F:=F PrimeField.M_pos).
  Instance spec_of_bn256_inv : spec_of PrimeField.inv :=
    AbstractField.unop_spec AbstractField.un_inv (F:=F PrimeField.M_pos).
  Instance spec_of_bn256_copy : spec_of PrimeField.felem_copy :=
    AbstractField.spec_of_felem_copy (F:=F PrimeField.M_pos).

  (* spec_of instances for Fp2 operations *)
  Instance spec_of_bn256_Fp2_add : spec_of (AbstractField.add (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
    AbstractField.binop_spec AbstractField.bin_add (F:=F PrimeField.M_pos * F PrimeField.M_pos).
  Instance spec_of_bn256_Fp2_sub : spec_of (AbstractField.sub (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
    AbstractField.binop_spec AbstractField.bin_sub (F:=F PrimeField.M_pos * F PrimeField.M_pos).
  Instance spec_of_bn256_Fp2_mul : spec_of (AbstractField.mul (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
    AbstractField.binop_spec AbstractField.bin_mul (F:=F PrimeField.M_pos * F PrimeField.M_pos).
  Instance spec_of_bn256_Fp2_copy : spec_of (AbstractField.felem_copy (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
  ...
  Lemma bn256_mulp2_eq_fp2_mul : forall a b,
    QuadraticExtensions.mulp2 PrimeField.M_pos bn256_beta a b =
    Fp6.fp2_mul PrimeField.M_pos bn256_beta a b.
  ...
  Let ftfst := @Field.field_theory_for_stdlib_tactic
    ...
    (@F.field_modulo PrimeField.M_pos prime_bn256).
  Add Field Fp_field : ftfst.

  Let FFp2 := QuadraticExtensions.FFp2 PrimeField.M_pos prime_bn256
    bn256_M_big bn256_beta bn256_beta_nz bn256_beta_qnr.
      ...
      { assert (@F.sub PrimeField.M_pos (@F.mul PrimeField.M_pos a0 a0)
        ...
        assert (@F.mul PrimeField.M_pos a0 a0 =
                @F.sub PrimeField.M_pos (@F.zero PrimeField.M_pos)
                  (@F.opp PrimeField.M_pos (@F.mul PrimeField.M_pos (@F.mul PrimeField.M_pos bn256_beta a1) a1))).
        { rewrite <- H. ring. }
        rewrite H0. ring. }
  ...
  Lemma bn256_invp2_eq_fp2_inv : forall x,
    x <> QuadraticExtensions.zerop2 PrimeField.M_pos ->
    QuadraticExtensions.invp2 PrimeField.M_pos bn256_beta x =
    Fp6.fp2_inv PrimeField.M_pos bn256_beta x.
  Proof.
    intros [a0 a1] Hx.
    pose proof (bn256_norm_nonzero a0 a1 Hx) as Hnorm.
    assert (Hinvp2 : QuadraticExtensions.mulp2 PrimeField.M_pos bn256_beta (a0,a1)
      (QuadraticExtensions.invp2 PrimeField.M_pos bn256_beta (a0,a1)) =
      QuadraticExtensions.onep2 PrimeField.M_pos).
    { pose proof (Finv_l FFp2 (a0,a1) Hx). rewrite <- H. ring. }
    assert (Hfpinv : QuadraticExtensions.mulp2 PrimeField.M_pos bn256_beta (a0,a1)
      (Fp6.fp2_inv PrimeField.M_pos bn256_beta (a0,a1)) =
      QuadraticExtensions.onep2 PrimeField.M_pos).
    ...
    assert (Hy2 : Fp6.fp2_inv PrimeField.M_pos bn256_beta (a0, a1) =
                  QuadraticExtensions.invp2 PrimeField.M_pos bn256_beta (a0, a1)).
    { transitivity (QuadraticExtensions.mulp2 PrimeField.M_pos bn256_beta
        (QuadraticExtensions.onep2 PrimeField.M_pos)
        (Fp6.fp2_inv PrimeField.M_pos bn256_beta (a0, a1))). { ring. }
      rewrite <- Hinv_l.
      transitivity (QuadraticExtensions.mulp2 PrimeField.M_pos bn256_beta
        (QuadraticExtensions.invp2 PrimeField.M_pos bn256_beta (a0, a1))
        (QuadraticExtensions.mulp2 PrimeField.M_pos bn256_beta (a0, a1)
          (Fp6.fp2_inv PrimeField.M_pos bn256_beta (a0, a1)))). { ring. }
      rewrite Hfpinv. ring. }
    symmetry. exact Hy2.
  Qed.
  ...
  Instance bn256_Fp6_params : AbstractField.FieldParameters
    ((F PrimeField.M_pos * F PrimeField.M_pos) *
     (F PrimeField.M_pos * F PrimeField.M_pos) *
     (F PrimeField.M_pos * F PrimeField.M_pos)) :=
    Fp6_field_parameters bn256_beta bn256_xi_re bn256_xi_im (fp6_prefix:=fp6_prefix).

  Instance bn256_Fp6_rep : AbstractField.FieldRepresentation (F:=
    (F PrimeField.M_pos * F PrimeField.M_pos) *
    (F PrimeField.M_pos * F PrimeField.M_pos) *
    (F PrimeField.M_pos * F PrimeField.M_pos)) :=
    Fp6_field_representation bn256_beta bn256_xi_re bn256_xi_im (fp6_prefix:=fp6_prefix) (fp2_prefix:=fp2_prefix).
