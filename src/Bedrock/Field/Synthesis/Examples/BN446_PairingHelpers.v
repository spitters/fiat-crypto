...
(* ================================================================ *)
(* BN446 Section context — mirrors BN446_Pairing.v                  *)
(* ================================================================ *)
...
Section BN446_PairingHelpers.
    ...
    (* beta = -1 for BN446 (p ≡ 3 mod 4) *)
    Let bn446_beta : F PrimeField.M_pos := F.of_Z PrimeField.M_pos (-1).

    (* xi = (9, 1) for BN446 (cubic non-residue in Fp2 for Fp6 tower) *)
    ...
    (* The function body defined here (via bn446_pf_params) must equal
       the imported one from BN446_Pairing (via bn446_prime_params).
       Both define M_pos = Z.to_pos bn446_prime.m and the same function names,
    ...
    Instance spec_of_Fp_opp : spec_of (@AbstractField.opp _ prime_field_parameters) :=
      AbstractField.unop_spec (F:=Fp) (field_parameters:=prime_field_parameters)
        (field_representation:=bn446_Fp_rep) AbstractField.un_opp.
    ...
    Instance spec_of_bn446_Fp2_mul_fp : spec_of "bn446_Fp2_mul_fp" :=
      fnspec! "bn446_Fp2_mul_fp" (pout px ps : word)
    ...
    Lemma bn446_Fp2_mul_fp_ok :
      forall functions
        (EnvContains : map.get functions "bn446_Fp2_mul_fp" =
          Some (snd bn446_Fp2_mul_fp))
        (HFmul : spec_of_Fp_mul functions),
      spec_of_bn446_Fp2_mul_fp functions.
    Proof.
      intros.
      unfold spec_of_bn446_Fp2_mul_fp.
      ...
      unfold bn446_Fp2_mul_fp. simpl snd. simpl fst.
      cbv match beta.
      eexists. split. { exact eq_refl. }
      (* Step through: cmd.seq -> cmd.call arg evaluation *)
      straightline. straightline. straightline.
      (* Now at: call functions "bn446_mul" tr mem0 args postcondition *)
      ...
      pose proof (@QuadraticFieldExtensions.Fp2_raw_FElem_split _ _ _ _
        wordok mapok bn446_pf_params bn446_Fp_rep bn446_beta fp2_prefix
        px x m_x Hfx)
      ...
      change (@AbstractField.bounded_by _ bn446_Fp2_params' _ _ _ _ bn446_Fp2_rep')
        with (fun b ws => @AbstractField.bounded_by _ _ _ _ _ _ bn446_Fp_rep b
          (fst_felem ws)
          /\ @AbstractField.bounded_by _ _ _ _ _ _ bn446_Fp_rep b
          (snd_felem ws)) in Hbx.
      ...
      (* === Call 1: bn446_mul(out, x, s) — fst halves === *)
      ...
      1: { pose proof HFmul as HFmul1.
           ...
           split; [apply (@AbstractField.relax_bounds _ _ _ _ _ _
             bn446_Fp_rep bn446_Fp_rep_ok); exact Hbx1 |].
           split; [exact Hbs |].
      ...
      (* === Call 2: bn446_mul(out+off, x+off, s) — snd halves === *)
      eapply Semantics.weaken_call.
      1: { pose proof HFmul as HFmul2.
           ...
           split; [apply (@AbstractField.relax_bounds _ _ _ _ _ _
             bn446_Fp_rep bn446_Fp_rep_ok); exact Hbx2 |].
           split; [exact Hbs |].
      ...
      assert (Hlen_out1 : length out1 =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bn446_Fp_rep).
      { destruct Hsep1 as [mc [_ [_ [Hfc _]]]].
        exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bn446_pf_params bn446_Fp_rep _ _ _ Hfc). }
      ...
      assert (Hlen_x1 : length (fst_felem x) =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bn446_Fp_rep).
      { exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bn446_pf_params bn446_Fp_rep _ _ _ Hx1). }
      assert (Hlen_x2 : length (snd_felem x) =
        @AbstractField.felem_size_in_words _ _ _ _ _ _ bn446_Fp_rep).
      { exact (@QuadraticFieldExtensions.AbstractFElem_length _ _ _ _
          bn446_pf_params bn446_Fp_rep _ _ _ Hx2). }
      ...
      { (* Decompose Hsep2 to get individual maps *)
        ...
        rewrite (@QuadraticFieldExtensions.Fp2_list_decomp _ _ _ _
          bn446_pf_params bn446_Fp_rep x) in Hfp2_x.
        (* Build final sep *)
    ...
    Local Ltac solve_store_fp2_constant :=
      ...
      match goal with Hsep : (FElem_Fp2 _ _ ⋆ ?RR) _ |- _ =>
        ...
        match type of Hlen with length ?x = _ =>
          ...
          do_dest x; do_dest x; do_dest x; do_dest x; do_dest x; do_dest x; do_dest x;
          do_dest x; do_dest x; do_dest x; do_dest x; do_dest x; do_dest x; do_dest x;
      ...
      eexists (_ :: _ :: _ :: _ :: _ :: _ :: _ :: _ :: _ :: _ :: _ :: _ :: _ :: _ :: []);
      apply conj_swap;
      [ (* Sep: prove via Bignum_of_array_sep to avoid emp issues *)
        ...
        change (@AbstractField.felem_size_in_words
          _ bn446_Fp2_params' _ _ _ _ bn446_Fp2_rep') with 14%nat;
    ...
    Lemma bn446_load_gamma1_p2_ok :
      forall functions
        (EnvContains : map.get functions "bn446_load_gamma1_p2" =
          Some (snd bn446_load_gamma1_p2)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn446_load_gamma1_p2" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn446_load_gamma1_p2. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
      solve_store_fp2_constant.
    Qed.

    Lemma bn446_load_gamma2_p2_ok :
      forall functions
        (EnvContains : map.get functions "bn446_load_gamma2_p2" =
          Some (snd bn446_load_gamma2_p2)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn446_load_gamma2_p2" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn446_load_gamma2_p2. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
      solve_store_fp2_constant.
    Qed.

    Lemma bn446_load_w_frob_p2_c1_ok :
      forall functions
        (EnvContains : map.get functions "bn446_load_w_frob_p2_c1" =
          Some (snd bn446_load_w_frob_p2_c1)),
      forall pout (old_out : Fp2_felem) Rr tr mem,
        (FElem_Fp2 pout old_out ⋆ Rr) mem ->
        WeakestPrecondition.call functions "bn446_load_w_frob_p2_c1" tr mem [pout]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              Fp2_bounded Fp2_tight out /\
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn446_load_w_frob_p2_c1. simpl snd. simpl fst. cbv match beta.
      eexists. split. { exact eq_refl. }
      solve_store_fp2_constant.
    Qed.
    ...
    Local Notation fp2_felem_offset :=
      (Memory.bytes_per_word 64 * Z.of_nat (@AbstractField.felem_size_in_words _ bn446_Fp2_params' _ _ _ _ bn446_Fp2_rep')).
    Local Notation Fp6_felem_size := (@AbstractField.felem_size_in_words _ bn446_Fp6_params' _ _ _ _ bn446_Fp6_rep').
    ...
    Lemma bn446_make_line_ok :
      forall functions
        (EnvContains : map.get functions "bn446_make_line" =
        ...
        (HFp2mulfp : spec_of_bn446_Fp2_mul_fp functions)
        (HFpcopy : spec_of_Fp_felem_copy functions)
        (HFfromword : spec_of_Fp_from_word functions),
      forall pout plam pxt pyt pxp pyp
        (old_out : @AbstractField.felem _ bn446_Fp12_params' _ _ _ _ bn446_Fp12_rep')
        ...
        WeakestPrecondition.call functions "bn446_make_line" tr mem
          [pout; plam; pxt; pyt; pxp; pyp]
          (fun tr' mem' rets =>
            rets = [] /\ tr = tr' /\
            exists out,
              @AbstractField.bounded_by _ bn446_Fp12_params' _ _ _ _ bn446_Fp12_rep'
                (@AbstractField.loose_bounds _ bn446_Fp12_params' _ _ _ _ bn446_Fp12_rep') out /\
              (FElem_Fp12 pout out ⋆
               (FElem_Fp2 plam lam ⋆
                (FElem_Fp2 pxt xt ⋆
                 (FElem_Fp2 pyt yt ⋆
    ...
    Proof.
      ...
      eapply start_func; [exact EnvContains | clear EnvContains].
      cbv [WeakestPrecondition.func].
      unfold bn446_make_line. simpl snd. simpl fst.
      cbv match beta.
      eexists. split. { exact eq_refl. }
      repeat straightline.
      ...
      set (FE2 := @AbstractField.FElem _
        (Fp2_field_parameters bn446_beta fp2_prefix) _ _ _ _
        (Fp2_field_representation bn446_beta fp2_prefix)).
      ...
      change (Fp2_field_parameters bn446_beta fp2_prefix)
        with bn446_Fp2_params' in Hsep.
      change (Fp2_field_representation bn446_beta fp2_prefix)
        with bn446_Fp2_rep' in Hsep.

      (* Fp2-level relax_bounds helper *)
      pose proof (@Fp2_field_representation_ok _ _ _ _ bn446_pf_params
        bn446_Fp_rep bn446_Fp_rep_ok bn446_beta fp2_prefix) as Fp2_rep_ok.
      ...
      1: { pose proof HFp2mul as HF1.
           ...
           split; [apply (@AbstractField.relax_bounds _ bn446_Fp2_params' _ _ _ _
             bn446_Fp2_rep' Fp2_rep_ok); exact Hblam |].
           split; [apply (@AbstractField.relax_bounds _ bn446_Fp2_params' _ _ _ _
             bn446_Fp2_rep' Fp2_rep_ok); exact Hbxt |].
      ...
      1: { pose proof HFp2mulfp as HF3.
           unfold spec_of_bn446_Fp2_mul_fp in HF3.
      ...
      *)
      ...
      { (* bounded_by loose_bounds the_out *)
        ...
        unfold Fp2_bounded, AbstractField.bounded_by,
               bn446_Fp2_rep', bn446_Fp2_params',
        ...
        pose proof (@AbstractField.relax_bounds _ _ _ _ _ _
          bn446_Fp_rep bn446_Fp_rep_ok) as RB.
        ...
        Local Typeclasses Transparent bn446_Fp12_params'.
        Local Typeclasses Transparent bn446_Fp12_rep'.
        Local Typeclasses Transparent bn446_Fp6_params'.
        Local Typeclasses Transparent bn446_Fp6_rep'.
        Local Typeclasses Transparent bn446_Fp2_params'.
        Local Typeclasses Transparent bn446_Fp2_rep'.
        cbv [AbstractField.bounded_by AbstractField.loose_bounds
             bn446_Fp12_rep' bn446_Fp12_params'
             bn446_Fp6_rep' bn446_Fp6_params'
             bn446_Fp2_rep' bn446_Fp2_params'
             bn446_beta bn446_xi_re bn446_xi_im
        ...
        split.
        (* d0 bound *)
        { change (DodecicFieldExtensionsSpecs.Fp6_repr_inst bn446_beta bn446_xi_re bn446_xi_im) with bn446_Fp6_rep'.
          ...
          cbv [DodecicFieldExtensionsSpecs.Fp6_repr_inst bn446_beta bn446_xi_re bn446_xi_im
               CubicFieldExtensionsSpecs.Fp6_field_representation
               CubicFieldExtensionsSpecs.Fp6_field_parameters
               CubicFieldExtensionsSpecs.c0_felem
               CubicFieldExtensionsSpecs.c1_felem
