...
(* BN446: 446-bit prime, 7 x 64-bit words *)
Section Field.
  Definition n : nat := 7.
  Definition m : Z := bn446_modulus.
  ...
  Definition prefix : string := "bn446_"%string.

  Instance bn446_field_parameters : FieldParameters.
  Proof using Type.
    let M := (eval vm_compute in (Z.to_pos m)) in
    let a := constr:(F.of_Z M 0) in
    let prefix := constr:("bn446_"%string) in
    eapply
      (field_parameters_prefixed
         M a prefix).
  Defined.

  Instance bn446_field_parameters_ok : FieldParameters_ok.
  Proof using Type.
    constructor.
    exact prime_bn446.
  Qed.
  ...
  Instance bn446_ops : @word_by_word_Montgomery_ops from_mont_string to_mont_string _ _ _ _ _ _ _ _ _ _ (WordByWordMontgomery.n m machine_wordsize) m.
  Proof using Type. Time constructor; make_computed_op. Defined.

  Instance bn446_frep : FieldRepresentation := field_representation m.
  Instance bn446_frep_raw : FieldRepresentation := field_representation_raw m.
  Instance bn446_frep_ok : FieldRepresentation_ok (field_representation:=bn446_frep).
  Proof.
    ...
    Lia.lia.
  Defined.
  Instance bn446_frep_raw_ok : FieldRepresentation_ok (field_representation:=bn446_frep_raw).
  Proof.
    apply Crypto.Bedrock.Field.Synthesis.New.Signature.field_representation_ok.
    intros. assumption.
    let c := eval lazy in felem_size_in_bytes in change felem_size_in_bytes with c.
  ...
  Derive bn446_from_bytes
         SuchThat (forall functions,
                      functions_contain functions bn446_from_bytes ->
                      spec_of_from_bytes
                        (field_representation:=bn446_frep_raw)
                        functions)
         As bn446_from_bytes_correct.
  Proof. Time derive_bedrock2_func from_bytes_op. Qed.

  Derive bn446_to_bytes
         SuchThat (forall functions,
                      functions_contain functions bn446_to_bytes ->
                      spec_of_to_bytes
                        (field_representation:=bn446_frep_raw)
                        functions)
         As bn446_to_bytes_correct.
  Proof. Time derive_bedrock2_func to_bytes_op. Qed.

  Derive bn446_mul
         SuchThat (forall functions,
                      functions_contain functions bn446_mul ->
                      spec_of_BinOp bin_mul
                        (field_representation:=bn446_frep)
                        functions)
         As bn446_mul_correct.
  Proof. Time derive_bedrock2_func mul_op. Qed.

  Derive bn446_square
         SuchThat (forall functions,
                      functions_contain functions bn446_square ->
                      spec_of_UnOp un_square
                        (field_representation:=bn446_frep)
                        functions)
         As bn446_square_correct.
  Proof. Time derive_bedrock2_func square_op. Qed.

  Derive bn446_add
         SuchThat (forall functions,
                      functions_contain functions bn446_add ->
                      spec_of_BinOp bin_add
                        (field_representation:=bn446_frep)
                        functions)
         As bn446_add_correct.
  Proof. Time derive_bedrock2_func add_op. Qed.

  Derive bn446_sub
         SuchThat (forall functions,
                      functions_contain functions bn446_sub ->
                      spec_of_BinOp bin_sub
                        (field_representation:=bn446_frep)
                        functions)
         As bn446_sub_correct.
  Proof. Time derive_bedrock2_func sub_op. Qed.

  Derive bn446_from_mont
         SuchThat (forall functions,
                      functions_contain functions bn446_from_mont ->
                      spec_of_UnOp un_from_mont
                        (field_representation:=bn446_frep)
                        functions)
         As bn446_from_mont_correct.
  Proof.
    epair.
  ...
  Derive bn446_to_mont
         SuchThat (forall functions,
                      functions_contain functions bn446_to_mont ->
                      spec_of_UnOp un_to_mont
                        (field_representation:=bn446_frep)
                        functions)
         As bn446_to_mont_correct.
  Proof.
    epair.
  ...
  Derive bn446_select_znz
           SuchThat (forall functions,
                      functions_contain functions bn446_select_znz ->
                      spec_of_selectznz
                        (field_representation:=bn446_frep)
                        functions)
         As bn446_select_znz_correct.
  Proof.
    epair.
