(** * BN446 field tower instances (Fp2 -> Fp6 -> Fp12).

    Provides FieldParameters, FieldRepresentation, and spec_of instances
    for BN446 with beta=-1, xi=(2,3).
...
Require Import Crypto.Bedrock.Field.Synthesis.Examples.bn446_Fp2.
...
Section BN446.
  ...
  Existing Instances
    ...
    bn446_prime_parameters bn446_prime_parameters_ok
    bn446_field_representation bn446_field_representation_ok.
  ...
  Definition bn446_beta : F PrimeField.M_pos := F.of_Z PrimeField.M_pos (-1).
  Definition bn446_xi_re : F PrimeField.M_pos := F.of_Z PrimeField.M_pos 2.
  Definition bn446_xi_im : F PrimeField.M_pos := F.of_Z PrimeField.M_pos 3.
  ...
  Instance bn446_Fp2_params : AbstractField.FieldParameters (F PrimeField.M_pos * F PrimeField.M_pos) :=
    Fp2_field_parameters bn446_beta fp2_prefix.

  Instance bn446_Fp2_rep : AbstractField.FieldRepresentation (F:=F PrimeField.M_pos * F PrimeField.M_pos) :=
    Fp2_field_representation bn446_beta fp2_prefix.

  Instance bn446_Fp2_rep_ok : AbstractField.FieldRepresentation_ok (F:=F PrimeField.M_pos * F PrimeField.M_pos) :=
    Fp2_field_representation_ok bn446_beta fp2_prefix.

  Instance bn446_Fp2_names : FieldNames (F:=F PrimeField.M_pos * F PrimeField.M_pos) :=
    field_names_prefixed fp2_prefix.

  (* spec_of instances for Fp-level operations *)
  Instance spec_of_bn446_add : spec_of PrimeField.add :=
    AbstractField.binop_spec AbstractField.bin_add (F:=F PrimeField.M_pos).
  Instance spec_of_bn446_sub : spec_of PrimeField.sub :=
    AbstractField.binop_spec AbstractField.bin_sub (F:=F PrimeField.M_pos).
  Instance spec_of_bn446_mul : spec_of PrimeField.mul :=
    AbstractField.binop_spec AbstractField.bin_mul (F:=F PrimeField.M_pos).
  Instance spec_of_bn446_sqr : spec_of PrimeField.square :=
    AbstractField.unop_spec AbstractField.un_square (F:=F PrimeField.M_pos).
  Instance spec_of_bn446_inv : spec_of PrimeField.inv :=
    AbstractField.unop_spec AbstractField.un_inv (F:=F PrimeField.M_pos).
  Instance spec_of_bn446_copy : spec_of PrimeField.felem_copy :=
    AbstractField.spec_of_felem_copy (F:=F PrimeField.M_pos).

  (* spec_of instances for Fp2 operations *)
  Instance spec_of_bn446_Fp2_add : spec_of (AbstractField.add (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
    AbstractField.binop_spec AbstractField.bin_add (F:=F PrimeField.M_pos * F PrimeField.M_pos).
  Instance spec_of_bn446_Fp2_sub : spec_of (AbstractField.sub (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
    AbstractField.binop_spec AbstractField.bin_sub (F:=F PrimeField.M_pos * F PrimeField.M_pos).
  Instance spec_of_bn446_Fp2_mul : spec_of (AbstractField.mul (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
    AbstractField.binop_spec AbstractField.bin_mul (F:=F PrimeField.M_pos * F PrimeField.M_pos).
  Instance spec_of_bn446_Fp2_copy : spec_of (AbstractField.felem_copy (F:=F PrimeField.M_pos * F PrimeField.M_pos)) :=
  ...
  Lemma bn446_mulp2_eq_fp2_mul : forall a b,
    QuadraticExtensions.mulp2 PrimeField.M_pos bn446_beta a b =
    Fp6.fp2_mul PrimeField.M_pos bn446_beta a b.
  ...
  Let ftfst := @Field.field_theory_for_stdlib_tactic
    ...
    (@F.field_modulo PrimeField.M_pos prime_bn446).
  Add Field Fp_field : ftfst.

  Let FFp2 := QuadraticExtensions.FFp2 PrimeField.M_pos prime_bn446
    bn446_M_big bn446_beta bn446_beta_nz bn446_beta_qnr.
      ...
      { assert (@F.sub PrimeField.M_pos (@F.mul PrimeField.M_pos a0 a0)
        ...
        assert (@F.mul PrimeField.M_pos a0 a0 =
                @F.sub PrimeField.M_pos (@F.zero PrimeField.M_pos)
                  (@F.opp PrimeField.M_pos (@F.mul PrimeField.M_pos (@F.mul PrimeField.M_pos bn446_beta a1) a1))).
        { rewrite <- H. ring. }
        rewrite H0. ring. }
  ...
  Lemma bn446_invp2_eq_fp2_inv : forall x,
    x <> QuadraticExtensions.zerop2 PrimeField.M_pos ->
    QuadraticExtensions.invp2 PrimeField.M_pos bn446_beta x =
    Fp6.fp2_inv PrimeField.M_pos bn446_beta x.
  Proof.
    intros [a0 a1] Hx.
    pose proof (bn446_norm_nonzero a0 a1 Hx) as Hnorm.
    assert (Hinvp2 : QuadraticExtensions.mulp2 PrimeField.M_pos bn446_beta (a0,a1)
      (QuadraticExtensions.invp2 PrimeField.M_pos bn446_beta (a0,a1)) =
      QuadraticExtensions.onep2 PrimeField.M_pos).
    { pose proof (Finv_l FFp2 (a0,a1) Hx). rewrite <- H. ring. }
    assert (Hfpinv : QuadraticExtensions.mulp2 PrimeField.M_pos bn446_beta (a0,a1)
      (Fp6.fp2_inv PrimeField.M_pos bn446_beta (a0,a1)) =
      QuadraticExtensions.onep2 PrimeField.M_pos).
    ...
    assert (Hy2 : Fp6.fp2_inv PrimeField.M_pos bn446_beta (a0, a1) =
                  QuadraticExtensions.invp2 PrimeField.M_pos bn446_beta (a0, a1)).
    { transitivity (QuadraticExtensions.mulp2 PrimeField.M_pos bn446_beta
        (QuadraticExtensions.onep2 PrimeField.M_pos)
        (Fp6.fp2_inv PrimeField.M_pos bn446_beta (a0, a1))). { ring. }
      rewrite <- Hinv_l.
      transitivity (QuadraticExtensions.mulp2 PrimeField.M_pos bn446_beta
        (QuadraticExtensions.invp2 PrimeField.M_pos bn446_beta (a0, a1))
        (QuadraticExtensions.mulp2 PrimeField.M_pos bn446_beta (a0, a1)
          (Fp6.fp2_inv PrimeField.M_pos bn446_beta (a0, a1)))). { ring. }
      rewrite Hfpinv. ring. }
    symmetry. exact Hy2.
  Qed.
  ...
  Instance bn446_Fp6_params : AbstractField.FieldParameters
    ((F PrimeField.M_pos * F PrimeField.M_pos) *
     (F PrimeField.M_pos * F PrimeField.M_pos) *
     (F PrimeField.M_pos * F PrimeField.M_pos)) :=
    Fp6_field_parameters bn446_beta bn446_xi_re bn446_xi_im (fp6_prefix:=fp6_prefix).

  Instance bn446_Fp6_rep : AbstractField.FieldRepresentation (F:=
    (F PrimeField.M_pos * F PrimeField.M_pos) *
    (F PrimeField.M_pos * F PrimeField.M_pos) *
    (F PrimeField.M_pos * F PrimeField.M_pos)) :=
    Fp6_field_representation bn446_beta bn446_xi_re bn446_xi_im (fp6_prefix:=fp6_prefix) (fp2_prefix:=fp2_prefix).
