(* BN256 Fp2 = Fp[u]/(u^2 + 1) where beta = -1 is a QNR.
   ...
   Fp6 nonresidue xi = 3 + u = (3, 1) in Fp2
   mul_xi: (a+bu)*(3+u) = (3a - b) + (a + 3b)u *)
...
Section bn256_Fp2.
    ...
    Instance bn256_prime_parameters : PrimeFieldParameters := {|
    ...
    Instance bn256_prime_parameters_ok : PrimeFieldParameters_ok.
    Proof. constructor. exact prime_bn256. Qed.

    Existing Instance prime_field_parameters.

    Instance bn256_field_representation : AbstractField.FieldRepresentation
    ...
    Instance bn256_field_representation_ok : AbstractField.FieldRepresentation_ok
      (F:=F PrimeField.M_pos).
    Proof.
      constructor. intros X H.
      cbv [bounded_by bn256_field_representation] in *.
      cbv [Field.bounded_by bn256_frep field_representation
           Signature.field_representation Representation.frep] in *.
      exact H.
    Defined.

    Instance bn256_field_names : FieldNames (F:=F PrimeField.M_pos) :=
    ...
    Definition Fp2_felem_copy : string * Syntax.func :=
      ("bn256_Fp2_felem_copy", (["out"; "x"], []:list String.string, bedrock_func_body:(
    ...
    Definition Fp2_add : string * Syntax.func :=
      ("bn256_Fp2_add", (["out"; "inx"; "iny"], []:list String.string, bedrock_func_body:(
    ...
    Definition Fp2_sub : string * Syntax.func :=
      ("bn256_Fp2_sub", (["out"; "inx"; "iny"], []:list String.string, bedrock_func_body:(
    ...
    Definition Fp2_zero : string * Syntax.func :=
      ("bn256_Fp2_zero", (["out"], []:list String.string, bedrock_func_body:(
    ...
    Definition Fp2_one : string * Syntax.func :=
      ("bn256_Fp2_one", (["out"], []:list String.string, bedrock_func_body:(
    ...
    (* beta = -1: ac - bd replaces ac + beta*bd, a^2 - b^2 replaces     *)
    (*            a^2 + beta*b^2                                         *)
    (* ================================================================ *)
    ...
    Definition Fp2_mul : string * Syntax.func :=
      ("bn256_Fp2_mul", (["out"; "inx"; "iny"], []:list String.string, bedrock_func_body:(
        ...
        (* out.re = v0 - v1 = ac - bd  (beta = -1, so ac + beta*bd = ac - bd) *)
    ...
    Definition Fp2_sqr : string * Syntax.func :=
      ("bn256_Fp2_square", (["out"; "inx"], []:list String.string, bedrock_func_body:(
        stackalloc (AbstractField.felem_size_in_bytes (F:=F)) as v0;
        stackalloc (AbstractField.felem_size_in_bytes (F:=F)) as v1;
        (* v0 = a^2 *)
        ...
        (* out.re = a^2 - b^2  (beta = -1, so a^2 + beta*b^2 = a^2 - b^2) *)
        coq:(cmd.call [] (AbstractField.sub (F:=F)) [expr.var "out"; expr.var "v0"; expr.var "v1"])
      ))).
    ...
    Definition Fp2_inv : string * Syntax.func :=
      ("bn256_Fp2_inv", (["out"; "inx"], []:list String.string, bedrock_func_body:(
    ...
    (* ================================================================ *)
    (* fp2_mul_xi: multiply by xi = (3, 1) in Fp2                       *)
    (* (a + bu)*(3 + u) = (3a + beta*b) + (a + 3b)u = (3a - b) + (a + 3b)u *)
    (* Multiply-by-3: x -> 2x -> 2x+x = 3x  (2 adds)                   *)
    (* ================================================================ *)

    Definition Fp2_mul_xi : string * Syntax.func :=
      ("bn256_Fp2_mul_xi", (["out"; "x"], []:list String.string, bedrock_func_body:(
        ...
        (* out.re = 3a - b *)
        coq:(cmd.call [] (AbstractField.sub (F:=F)) [expr.var "out"; expr.var "tmp_a3"; expr_2nd_felem (expr.var "x")]);
        (* out.im = a + 3b *)
        coq:(cmd.call [] (AbstractField.add (F:=F)) [expr_2nd_felem (expr.var "out"); expr.var "x"; expr.var "tmp_b3"])
      ))).

    (* Fp2 conjugate: conj(a + bu) = a - bu *)
    Definition Fp2_conjugate : string * Syntax.func :=
      ("bn256_Fp2_conjugate", (["out"; "x"], []:list String.string, bedrock_func_body:(
        coq:(cmd.call [] (AbstractField.felem_copy (F:=F)) [expr.var "out"; expr.var "x"]);
