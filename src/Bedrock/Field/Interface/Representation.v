...
Section Representation.
  ...
  Context {F : Type} {prime_field_parameters : FieldParameters F}
          {p_ok : Types.ok}.
  ...
  Definition eval_words : list word -> F :=
    fun ws => FofZ (Positional.eval weight n (eval_transformation (map word.unsigned ws))).

  Definition eval_bytes : list byte -> F :=
    fun bs =>
      FofZ (Positional.eval
                           (ModOps.weight 8 1)
                           n_bytes 
                           (map byte.unsigned bs)).

  Local Instance frep : FieldRepresentation F := {
      feval := eval_words;
  ...
  Local Instance frep_ok : FieldRepresentation_ok F.
